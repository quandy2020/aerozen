...
void runSubscriber() {
    ...
    transport::Node::Subscriber sub =
        node.CreateSubscriber(g_topic, cbCreateSub);
    EXPECT_TRUE(sub);
    transport::Node::Subscriber sub2 =
        node.CreateSubscriber(g_topic, cbCreateSub2);
    EXPECT_TRUE(sub2);

    int interval = 100;

    // Wait until we've received at least one message in each callback.
    while (!cbExecuted || !cbRawExecuted || !cbCreateSubExecuted ||
           !cbCreateSub2Executed) {
