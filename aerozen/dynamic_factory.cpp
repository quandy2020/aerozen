...
#include "aerozen/installation_directories.hpp"
#include "aerozen/utils/environment.hpp"

namespace {
constexpr const char* kDescriptorEnv = "AEROZEN_DESCRIPTOR_PATH";
...
DynamicFactory::DynamicFactory() {
    ...
    if (aerozen::utils::env(kDescriptorEnv, descPaths)) {
        // Load all the descriptors found in the paths set with
        // AEROZEN_DESCRIPTOR_PATH.
        this->LoadDescriptors(descPaths);
    }

    auto globalPath = std::filesystem::path(aerozen::GetInstallPrefix()) /
                      "share" / "aerozen" / "protos";
...
void DynamicFactory::LoadDescriptors(const std::string& _paths) {
    ...
    auto loadDescFile = [this](const std::string& descFile) {
        ...
        if (!fileDescriptorSet.ParseFromIstream(&ifs)) {
            std::cerr
                << "DynamicFactory(): Unable to parse descriptor set from ["
                << descFile << "]" << std::endl;
            return;
        }
        ...
        for (const google::protobuf::FileDescriptorProto& fileDescriptorProto :
            ...
            if (!static_cast<bool>(this->pool.BuildFile(fileDescriptorProto))) {
                std::cerr
                    << "DynamicFactory(). Unable to place descriptors from ["
    ...
    for (const std::string& descDir : descDirs) {
        if (!std::filesystem::is_directory(descDir)) {
            ...
            for (auto const& dirIter :
                 std::filesystem::directory_iterator{descDir}) {
