...
TEST(RepStorageTest, RepStorageAPI) {
    ...
    std::map<std::string, std::map<std::string, IRepHandlerPtr>> m;
    HandlerStorage<IRepHandler> reps;
    msgs::Int32 rep1Msg;
    bool result;
    ...
    std::shared_ptr<RepHandler<msgs::Vector3d, msgs::Int32>> rep1HandlerPtr(
        new RepHandler<msgs::Vector3d, msgs::Int32>(pUuid1, nUuid1));
    ...
    std::shared_ptr<RepHandler<msgs::Int32, msgs::Int32>> rep2HandlerPtr(
        new RepHandler<msgs::Int32, msgs::Int32>(pUuid1, nUuid1));

    // Insert the handler.
    reps.AddHandler(topic, nUuid1, rep2HandlerPtr);

    // Create another REP handler without a callback for node1.
    std::shared_ptr<RepHandler<msgs::Int32, msgs::Int32>> rep5HandlerPtr(
        new RepHandler<msgs::Int32, msgs::Int32>(pUuid1, nUuid1));

    // Insert the handler.
    reps.AddHandler(topic, nUuid1, rep5HandlerPtr);
    EXPECT_TRUE(
        reps.RemoveHandler(topic, nUuid1, rep5HandlerPtr->HandlerUuid()));

    // Create a REP handler without a callback for node2.
    std::shared_ptr<RepHandler<msgs::Int32, msgs::Int32>> rep3HandlerPtr(
        new RepHandler<msgs::Int32, msgs::Int32>(pUuid1, nUuid1));
    ...
    std::shared_ptr<RepHandler<msgs::Int32, msgs::Int32>> rep4HandlerPtr(
        new RepHandler<msgs::Int32, msgs::Int32>(pUuid1, nUuid1));
