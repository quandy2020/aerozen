...
public:
    void SubscribeWithMessageInfo() {
        // Subscribe to an illegal topic.
        EXPECT_FALSE(
            this->node.Subscribe("Bad Topic", &MyTestClass::CbInfo, this));
...
public:
    void WithoutOutput(const msgs::Int32& _req) {
        EXPECT_EQ(_req.data(), data);
        this->callbackSrvExecuted = true;
    }
...
public:
    void TestServiceCall() {
        ...
        EXPECT_FALSE(
            this->node.Request(g_topic, wrongReq, timeout, rep, result));
        EXPECT_FALSE(
            this->node.Request(g_topic, req, timeout, wrongRep, result));
...
public:
    void TestServiceCallWithoutInput() {
        ...
        EXPECT_FALSE(
            this->node.Advertise("Bad Srv", &MyTestClass::WithoutInput, this));

        // Advertise and request a valid service without input.
        EXPECT_TRUE(
            this->node.Advertise(g_topic, &MyTestClass::WithoutInput, this));
        ...
        // Request a valid service without input using a member function
        // callback.
...
public:
    void TestServiceCallWithoutOutput() {
        ...
        EXPECT_FALSE(
            this->node.Advertise("Bad Srv", &MyTestClass::WithoutOutput, this));

        // Advertise and request a valid service without waiting for response.
        EXPECT_TRUE(
            this->node.Advertise(g_topic, &MyTestClass::WithoutOutput, this));
...
TEST(NodeTest, PubWithoutAdvertise) {
    ...
    // Check that an invalid namespace is ignored. The callbacks are expecting
    // an empty namespace.
...
TEST(NodeTest, PubSubSameThreadLambdaMessageInfo) {
    ...
    bool executed = false;
    std::function<void(const msgs::Int32&, const transport::MessageInfo&)>
        subCb = [&executed](const msgs::Int32& _msg,
                            const transport::MessageInfo& _info) {
            EXPECT_EQ(_info.Topic(), g_topic);
            EXPECT_EQ(_msg.data(), data);
            EXPECT_EQ(g_FQNPartition, _info.Partition());
...
TEST(NodeSubTest, BoolOperatorTest) {
    ...
    std::function<void(const msgs::Int32&)> cb = [](const msgs::Int32&) {};
...
TEST(NodeTest, ServiceCallAsyncLambda) {
    ...
    std::function<bool(const msgs::Int32&, msgs::Int32&)> advCb =
    ...
    transport::Node node;
    EXPECT_TRUE((node.Advertise<msgs::Int32, msgs::Int32>(g_topic, advCb)));
...
TEST(NodeTest, ServiceCallWithoutInputAsyncLambda) {
    ...
    std::function<bool(msgs::Int32&)> advCb = [](msgs::Int32& _rep) -> bool {
        _rep.set_data(data);
        return true;
    };
...
TEST(NodeTest, ServiceCallSyncTimeout) {
    ...
    bool executed = node.Request(
        g_topic, req, static_cast<unsigned int>(timeout), rep, result);
