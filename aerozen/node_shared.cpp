...
NodeShared::NodeShared() : dataPtr(new NodeSharedPrivate) {
    ...
    this->dataPtr->msgDiscovery->DisconnectionsCb(std::bind(
        &NodeShared::OnNewDisconnection, this, std::placeholders::_1));
    ...
    this->dataPtr->srvDiscovery->ConnectionsCb(std::bind(
        &NodeShared::OnNewSrvConnection, this, std::placeholders::_1));
    ...
    if (impl == "zeromq") {
        ...
        if (this->dataPtr->verbose) {
            std::cout << "Host address: " << this->dataPtr->hostAddr
                      << std::endl;
            ...
            std::cout << "Identity for receiving srv. responses: ["
                      << this->responseReceiverId.ToString() << "]"
                      << std::endl;
        }

        // Start the ZMQ reception thread.
        this->threadReception =
            std::thread(&NodeShared::RunReceptionTask, this);

        // Set the callback to notify svc discovery updates (invalid services).
        this->dataPtr->srvDiscovery->DisconnectionsCb(std::bind(
            &NodeShared::OnNewSrvDisconnection, this, std::placeholders::_1));
        ...
        this->dataPtr->msgDiscovery->Start(
            this->Session(), std::bind(&MsgDiscovery::LivelinessMsgDataHandler,
                                       this->dataPtr->msgDiscovery.get(),
                                       std::placeholders::_1));

        this->dataPtr->srvDiscovery->Start(
            this->Session(), std::bind(&SrvDiscovery::LivelinessSrvDataHandler,
                                       this->dataPtr->srvDiscovery.get(),
                                       std::placeholders::_1));
    }
#endif

    // Create the local publish thread.
    this->dataPtr->pubThread =
        std::thread(&NodeSharedPrivate::PublishThread, this->dataPtr.get());
...
void NodeShared::RunReceptionTask() {
    while (!this->dataPtr->exit) {
        ...
        zmq::pollitem_t items[] = {
            ...
            {static_cast<void*>(*this->dataPtr->responseReceiver), 0,
             ZMQ_POLLIN, 0}};
        try {
        ...
        try {
            ...
            if (this->dataPtr->topicStatsEnabled) {
                ...
                if (this->dataPtr->enabledTopicStatistics.find(topic) !=
                    ...
                    this->dataPtr->topicStats[topic].Update(sender, meta->stamp,
                                                            meta->seq);
...
NodeShared::HandlerInfo NodeShared::CheckHandlerInfo(
    ...
    info.haveLocal =
        this->localSubscribers.normal.Handlers(_topic, info.localHandlers);

    info.haveRaw =
        this->localSubscribers.raw.Handlers(_topic, info.rawHandlers);
...
NodeShared::SubscriberInfo NodeShared::CheckSubscriberInfo(
    ...
    info.haveRemote = this->remoteSubscribers.HasTopic(_topic, _msgType);
            ...
            for (const auto& handler : node.second) {
                ...
                if (rawHandler) {
                    if (rawHandler->TypeName() == _info.Type() ||
                        ...
                        rawHandler->RunRawCallback(_msgData.c_str(),
                                                   _msgData.size(), _info);
                    }
                } else
                    std::cerr << "Raw subscription handler is NULL"
                              << std::endl;
            }
        }
    }
            ...
            for (const auto& handler : node.second) {
                ...
                if (localHandler) {
                    if (localHandler->TypeName() == _info.Type() ||
                        ...
                        if (!msg) {
                            // If the message has not been deserialized yet, do
                            // it now since we have allegedly found a subscriber
                            // which should be able to do it.
                            msg =
                                localHandler->CreateMsg(_msgData, _info.Type());

                            if (!msg) {
                                // If the message could not be created, then
                                // none of the handlers in this process will be
                                // able to create it, because protobuf has
                                // access to all message types that the current
                                // process is linked to. If CreateMsg(~,~)
                                // fails, then we may as well quit.
                                return;
                            }
                        }
                    ...
                    std::cerr << "Local subscription handler is NULL"
                              << std::endl;
            }
        }
    }
...
void NodeShared::RecvSrvRequest() {
...

    {
        ...
        hasHandler = this->dataPtr->repliers.FirstHandler(topic, reqType,
                                                          repType, repHandler);
    }
    ...
    if (hasHandler) {
...

        {
            ...
            if (std::find(this->srvConnections.begin(),
                          this->srvConnections.end(),
...
void NodeShared::RecvSrvResponse() {
...

    {
        ...
        hasHandler = this->dataPtr->requests.Handler(topic, nodeUuid, reqUuid,
                                                     reqHandlerPtr);
    }
    ...
    if (hasHandler) {
        ...
        std::lock_guard<std::recursive_mutex> lock(this->mutex);
        {
            if (!this->dataPtr->requests.RemoveHandler(topic, nodeUuid,
                                                       reqUuid)) {
        ...
        std::cerr
            << "Received a service call response but I don't have a handler"
            << " for it" << std::endl;
    }
}
...
void NodeShared::SendPendingRemoteReqs(const std::string& _topic,
    ...
    // Find a publisher that offers this service with a particular pair of
    // REQ/REP types.
    bool found = false;
    for (auto& proc : addresses) {
        auto& v = proc.second;
        for (auto& pub : v) {
            if (pub.ReqTypeName() == _reqType &&
                pub.RepTypeName() == _repType) {
                found = true;
    ...
    if (dataPtr->verbose) {
        std::cout << "Found a service call responser at [" << responserAddr
                  << "]" << std::endl;
    }
    ...
    for (auto& node : reqs) {
        for (auto& req : node.second) {
            ...
            // Check that the pending service call has types that match the
            // responser.
...
#ifdef GZ_ZMQ_POST_4_3_1
                    this->dataPtr->requester->send(msg,
            ...
            if (_repType == msgs::Empty().GetTypeName()) {
                this->dataPtr->requests.RemoveHandler(_topic, nodeUuid,
                                                      reqUuid);
            }
        }
    }
...
void NodeShared::OnNewConnection(const MessagePublisher& _pub) {
...

    {
        ...
        if (this->localSubscribers.HasSubscriber(topic) &&
...
#else
            this->dataPtr->subscriber->setsockopt(ZMQ_SUBSCRIBE, topic.data(),
                                                  topic.size());
#endif
            ...
            // Hack: We use this field to store the PUuid of the topic
            // publisher.
...
void NodeShared::OnNewDisconnection(const MessagePublisher& _pub) {
    ...
    if (topic != "" && nUuid != "") {
        ...
        // for this process. Remote nodes might suffer package delays (due to
        // WiFi or traffic load) and if we remove them, they won't be able to
...
void NodeShared::OnNewSrvConnection(const ServicePublisher& _pub) {
    ...
    if (this->dataPtr->requests.FirstHandler(topic, reqType, repType,
                                             handler)) {
        {
            std::unique_lock<std::mutex> queueLock(
                this->dataPtr->srvThreadMutex);
...
void NodeShared::OnNewSrvDisconnection(const ServicePublisher& _pub) {
    ...
    this->srvConnections.erase(
        std::remove(std::begin(this->srvConnections),
...
void NodeShared::OnSubscribers() {
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(this->mutex);
        pubs = this->localSubscribers.Convert(this->dataPtr->myAddress,
                                              this->pUuid);
    }
...
bool NodeShared::InitializeSockets() {
    try {
...
#else
        this->dataPtr->publisher->setsockopt(ZMQ_LINGER, &lingerVal,
                                             sizeof(lingerVal));
#endif
...
#else
        this->dataPtr->subscriber->setsockopt(ZMQ_RCVHWM, &rcvQueueVal,
                                              sizeof(rcvQueueVal));
#endif
        ...
        this->dataPtr->myRequesterAddress =
            this->dataPtr->responseReceiver->get(zmq::sockopt::last_endpoint);
        ...
        this->dataPtr->publisher->setsockopt(ZMQ_SNDHWM, &sndQueueVal,
                                             sizeof(sndQueueVal));

        this->dataPtr->publisher->bind(anyTcpEp.c_str());
        size_t size = sizeof(bindEndPoint);
        this->dataPtr->publisher->getsockopt(ZMQ_LAST_ENDPOINT, &bindEndPoint,
                                             &size);
        this->myAddress = bindEndPoint;

        // ResponseReceiver socket listening in a random port.
        std::string id = this->responseReceiverId.ToString();
        this->dataPtr->responseReceiver->setsockopt(ZMQ_IDENTITY, id.c_str(),
                                                    id.size());
        this->dataPtr->responseReceiver->bind(anyTcpEp.c_str());
        ...
        int RouteOn = 1;
        this->dataPtr->replier->setsockopt(ZMQ_LINGER, &lingerVal,
                                           sizeof(lingerVal));
        this->dataPtr->replier->setsockopt(ZMQ_ROUTER_MANDATORY, &RouteOn,
                                           sizeof(RouteOn));
        this->dataPtr->replier->bind(anyTcpEp.c_str());
        this->dataPtr->replier->getsockopt(ZMQ_LAST_ENDPOINT, &bindEndPoint,
                                           &size);
        this->myReplierAddress = bindEndPoint;

        this->dataPtr->requester->setsockopt(ZMQ_LINGER, &lingerVal,
                                             sizeof(lingerVal));
...
bool NodeShared::HandlerWrapper::HasSubscriber(
    ...
    return this->normal.FirstHandler(_fullyQualifiedTopic, _msgType,
                                     normalSubscriberPtr) ||
           this->raw.FirstHandler(_fullyQualifiedTopic, _msgType,
                                  rawSubscriberPtr);
}
...
#else
        this->subscriber->setsockopt(ZMQ_PLAIN_USERNAME, user.c_str(),
                                     user.size());
        this->subscriber->setsockopt(ZMQ_PLAIN_PASSWORD, pass.c_str(),
                                     pass.size());
#endif
    }
}
...
void NodeSharedPrivate::SecurityInit() {
    ...
    if (userPass(user, pass)) {
        ...
        this->accessControlThread =
            std::thread(&NodeSharedPrivate::AccessControlHandler, this);
...
void NodeSharedPrivate::AccessControlHandler() {
    ...
    try {
        ...
        while (!this->exit) {
            try {
                zmq::poll(
                    &items[0], sizeof(items) / sizeof(items[0]),
                    std::chrono::milliseconds(NodeSharedPrivate::Timeout));
            } catch (...) {
                continue;
            }
            ...
            if (items[0].revents & ZMQ_POLLIN) {
                ...
                // eliminate unused variable warning when debug statements are
                // commented
                (void)routingId;
                ...
                // Check that we received some kind of address. This could be
                // used in the future to only accept connections from specific
                // addresses.
                if (address.empty()) {
            ...
            try {
                ...
                if (std::string str;
                    ...
                    std::cerr << "Exception occurred in a local callback "
                              << "on topic [" << msgDetails->info.Topic()
                              << "] with message [" << str << "]" << std::endl;
                } else {
                    std::cerr << "Exception occurred in a local callback "
                              << "on topic [" << msgDetails->info.Topic()
                              << "]. Unable to show msg" << std::endl;
                }
            }
        }
...
void NodeShared::SrvPublishThread() {
    ...
    while (!this->dataPtr->exit) {
        ...
        // Lock the mutex, and acquire the next publisher to be checked.
        {
            ...
            // Wait for more srv publishers if the queue is empty. Otherwise get
            // the next publisher and continue.
            if (this->dataPtr->srvQueue.empty()) {
                auto now = std::chrono::system_clock::now();
                this->dataPtr->signalNewSrv.wait_until(
                    queueLock, now + 500ms, [&] {
                        return !this->dataPtr->srvQueue.empty() ||
                               this->dataPtr->exit;
                    });
            }
...
int NodeSharedPrivate::NonNegativeEnvVar(const std::string& _envVar,
    ...
    if (env(_envVar, strVal)) {
        try {
            ...
            std::cerr << "Unable to convert " << _envVar << " value [" << strVal
                      << "] to an integer number. Using [" << _defaultValue
                      << "] instead." << std::endl;
        } catch (std::out_of_range&) {
            std::cerr << "Unable to convert " << _envVar << " value [" << strVal
                      << "] to an integer number. This number is "
        ...
        if (numVal < 0) {
            ...
            std::cerr << "Unable to convert " << _envVar << " value [" << strVal
                      << "] to a non-negative number. This number is "
...
bool NodeShared::Unsubscribe(const std::string& _topic,
    ...
    if (!TopicUtils::FullyQualifiedName(_nOpt.Partition(), _nOpt.NameSpace(),
...

    {
        ...
        if (_hUuid.empty()) {
            this->localSubscribers.RemoveHandlersForNode(fullyQualifiedTopic,
                                                         _nUuid);
        } else {
            this->localSubscribers.RemoveHandler(fullyQualifiedTopic, _nUuid,
                                                 _hUuid);
        }

        // Check if there are still local subscribers in this node. If so then
        // we are done here. Otherwise, let others know that this node is no
        // longer interested in the topic
        if (this->localSubscribers.HasSubscriberForNode(fullyQualifiedTopic,
                                                        _nUuid)) {
            return true;
        }
    ...
    if (shouldNotifyPublishers) {
        ...
        this->dataPtr->msgDiscovery->Publishers(fullyQualifiedTopic, addresses);

        for (auto& proc : addresses) {
            std::string dstPUuid = proc.first;
            MessagePublisher pub(fullyQualifiedTopic, localAddress, dstPUuid,
                                 localPUuid, _nUuid, kGenericMessageType,
                                 AdvertiseMessageOptions());

            this->dataPtr->msgDiscovery->Unregister(pub);
        }

        MessagePublisher pub(fullyQualifiedTopic, localAddress, "", localPUuid,
                             _nUuid, kGenericMessageType,
                             AdvertiseMessageOptions());
...
bool NodeShared::SubscribeHelper(const std::string& _fullyQualifiedTopic,
    ...
    {
        std::lock_guard<std::recursive_mutex> lk(this->mutex);
        // Add the topic to the list of subscribed topics (if it was not
        // before).
...
bool NodeShared::RemoveHandlersFromPubQueue(const std::string& _topic,
    ...
    std::unique_lock<std::mutex> queueLock(this->dataPtr->pubThreadMutex);
