...
TEST(TopicUtilsTest, testTopics) {
    ...
    EXPECT_TRUE(TopicUtils::IsValidTopic("/abc"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("abc/de"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("a"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("abc/"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("/abc/"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("/abc/d"));
    EXPECT_TRUE(TopicUtils::IsValidTopic("/abc/d/e"));
    EXPECT_TRUE(
        TopicUtils::IsValidTopic(std::string(TopicUtils::kMaxNameLength, 'a')));

    EXPECT_FALSE(TopicUtils::IsValidTopic(""));
...
TEST(TopicUtilsTest, testFullyQualifiedName) {
    ...
    // text will be transformed. This value only makes sense if the text is
    // valid. E.g.: {"partition/",   {true,  "@/partition@"}} "partition/" is
    // valid text for a partition name and will be transformed into
    // "@/partition@" after calling to FullyQualifiedName().
    ...
    // Partitions to test.
    ValidationT partitions = {{"@partition", {false, ""}},
                              {"@partition/@", {false, ""}},
                              {"@@", {false, ""}},
                              {longString, {false, ""}},
    ...
    // Namespaces to test.
    ValidationT namespaces = {{"~ns", {false, ""}},
                              {longString, {false, ""}},
                              {"", {true, "/"}},
                              {"abc", {true, "/abc/"}},
                              {goodString, {true, "/" + goodString + "/"}}};
    ...
    ValidationT topics = {
        {"~/def", {false, ""}},          {"~/def/", {false, ""}},
        {"~def", {false, ""}},           {"~def/", {false, ""}},
        {longString, {false, ""}},       {"/def", {true, "/def"}},
        {"def/", {true, "def"}},         {"def/ghi", {true, "def/ghi"}},
        {"def/ghi/", {true, "def/ghi"}}, {goodString, {true, goodString}}};
    ...
    for (auto p : partitions)
        for (auto ns : namespaces)
            for (auto t : topics) {
                ...
                if (tUnderTest.front() == '/') {
                    isLongName =
                        p.second.second.size() + t.second.second.size() >
                        transport::TopicUtils::kMaxNameLength;
                } else {
                    isLongName = p.second.second.size() +
                                     ns.second.second.size() +
                                     t.second.second.size() >
                                 transport::TopicUtils::kMaxNameLength;
                }

                auto expectedRes = p.second.first && ns.second.first &&
                                   t.second.first && !isLongName;
                ...
                if (expectedRes) {
                    ...
                    // If the topic starts with "/", we should ignore the
                    // namespace.
                    if (tUnderTest.front() == '/')
...
TEST(TopicUtilsTest, asValidTopic) {
    for (auto unmodified : {"/abc", "abc/de", "a", "ABC/", "/abc", "/abc/d",
                            "/abc/d/e", "a(bc)d-e_f=h+i.j"}) {
...
TEST(TopicUtilsTest, MangleType) {
    ...
    EXPECT_TRUE(
        transport::TopicUtils::MangleType({"gz::msgs::StringMsg"}, output));
    EXPECT_EQ("gz::msgs::StringMsg", output);

    EXPECT_FALSE(transport::TopicUtils::MangleType({""}, output));
...
TEST(TopicUtilsTest, DemangleType) {
    ...
    EXPECT_TRUE(
        transport::TopicUtils::DemangleType({"gz::msgs::StringMsg"}, output));
    EXPECT_NE(std::find(output.begin(), output.end(), "gz::msgs::StringMsg"),
              output.end());

    EXPECT_FALSE(transport::TopicUtils::DemangleType({""}, output));
