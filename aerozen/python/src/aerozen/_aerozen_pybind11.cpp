...
PYBIND11_MODULE(BINDINGS_MODULE_NAME, m) {
    py::class_<AdvertiseOptions>(
        ...
        .def_property("scope", &AdvertiseOptions::Scope,
                      &AdvertiseOptions::SetScope,
                      "The scope used in this topic/service")
        .def(
            "__copy__",
            [](const AdvertiseOptions& self) { return AdvertiseOptions(self); })
    ...
    py::class_<AdvertiseMessageOptions, AdvertiseOptions>(
        ...
        .def_property_readonly("throttled", &AdvertiseMessageOptions::Throttled,
                               "Whether the publication has been throttled")
        .def_property(
            "msgs_per_sec", &AdvertiseMessageOptions::MsgsPerSec,
    ...
    py::class_<NodeOptions>(m, "NodeOptions",
                            "A class to provide different options for the node")
        .def(py::init<>())
        .def(py::init<const NodeOptions&>())
        .def_property("namespace", &NodeOptions::NameSpace,
                      &NodeOptions::SetNameSpace, "Set the node's namespace.")
        .def_property("partition", &NodeOptions::Partition,
                      &NodeOptions::SetPartition,
                      "Set the node's partition name.")
        .def("add_topic_remap", &NodeOptions::AddTopicRemap,
             py::arg("from_topic"), py::arg("to_topic"),
             "Add a new topic remapping. If a topic is remapped, the "
             "'from_topic'"
             "topic will be renamed to 'to_topic. Is not possible to add two "
             "remaps"
             "over the same 'from_topic'.")
        .def(
            "topic_remap",
            [](NodeOptions& self, const std::string& fromTopic) {
                std::string toTopic;
                bool result{false};
            ...
            py::arg("from_topic"),
            "Get a topic remapping. Returns a pair with the result of the "
            "method"
            "and the remapped name of the topic. The topic name remains empty "
            "if"
            "there isn't any remap for the topic")
        .def("__copy__",
             [](const NodeOptions& self) { return NodeOptions(self); })
        .def("__deepcopy__", [](const NodeOptions& self, pybind11::dict) {
    ...
    py::class_<SubscribeOptions>(
        ...
        .def_property_readonly("throttled", &SubscribeOptions::Throttled,
                               "Whether the subscription has been throttled")
        .def_property(
            "msgs_per_sec", &SubscribeOptions::MsgsPerSec,
            &SubscribeOptions::SetMsgsPerSec,
            "Set the maximum number of messages per second received per topic")
        .def(
            "__copy__",
            [](const SubscribeOptions& self) { return SubscribeOptions(self); })
        .def("__deepcopy__", [](const SubscribeOptions& self, pybind11::dict) {
            return SubscribeOptions(self);
        });

    // We are leaving this as an opaque class in order to be able to add
    // bindings to it later without breaking ABI.
    ...
    py::class_<MessagePublisher>(
        ...
        .def_property_readonly(
            "ctrl", &MessagePublisher::Ctrl,
            "Get the ZeroMQ control address of the publisher.")
        .def_property_readonly(
            "msg_type_name", &MessagePublisher::MsgTypeName,
            "Get the message type advertised by this publisher.")
        .def_property_readonly("options", &MessagePublisher::Options,
    ...
    py::class_<ServicePublisher>(
        ...
        .def(py::init<>())
        .def_property_readonly("socket_id", &ServicePublisher::SocketId,
                               "Get the ZeroMQ socket ID for this publisher.")
        .def_property_readonly(
            "req_type_name", &ServicePublisher::ReqTypeName,
            "Get the name of the request's protobuf message advertised.")
        .def_property_readonly("rep_type_name", &ServicePublisher::RepTypeName,
                               "Get the advertised options.")
        .def_property_readonly("options", &ServicePublisher::Options,
                               "Get the advertised options.");
    ...
    auto node =
        py::class_<Node>(
            ...
            .def(py::init<const NodeOptions&>())
            .def("advertise",
                 static_cast<Node::Publisher (Node::*)(
                     const std::string&, const std::string&,
                     const AdvertiseMessageOptions&)>(&Node::Advertise),
                 py::arg("topic"), py::arg("msg_type_name"), py::arg("options"),
                 "Advertise a new topic. If a topic is currently advertised,"
                 " you cannot advertise it a second time (regardless of its "
                 "type)")
            ...
            .def("unsubscribe", &Node::Unsubscribe, py::arg("topic"),
                 "Unsubscribe from a topic")
            // Send a service request using the blocking interface
            .def(
                "request_raw",
                [](Node& _node, const std::string& _service,
                   const std::string& _request, const std::string& _reqType,
                   const std::string& _repType, const unsigned int& _timeout) {
                    bool result{false};
                    std::string _response;
                    result =
                        _node.RequestRaw(_service, _request, _reqType, _repType,
                                         _timeout, _response, result);
                    return py::make_tuple(
                        result, py::bytes(_response.c_str(), _response.size()));
                },
                py::arg("topic"), py::arg("request"), py::arg("request_type"),
                py::arg("response_type"), py::arg("timeout"),
            ...
            .def(
                "topic_info",
                [](Node& _node, const std::string& _topic) {
                ...
                py::arg("topic"), "Get the information about a topic")
            .def(
                "service_list",
                [](Node& _node) {
            ...
            .def(
                ...
                py::arg("service"), "Get the information about a service")
            .def(
                "subscribe_raw",
                [](Node& _node, const std::string& _topic,
                   std::function<void(py::bytes _msgData,
                                      const MessageInfo& _info)>& _callback,
                   const std::string& _msgType, const SubscribeOptions& _opts) {
                    auto _cb = [_callback](const char* _msgData,
                                           const size_t _size,
    ...
    py::class_<gz::transport::Node::Publisher>(
        node, "Publisher",
        ...
        .def("publish_raw", &gz::transport::Node::Publisher::PublishRaw,
             py::arg("msg_data"), py::arg("msg_type"))
        .def("throttled_update_ready",
             &gz::transport::Node::Publisher::ThrottledUpdateReady, "")
        .def("has_connections", &gz::transport::Node::Publisher::HasConnections,
