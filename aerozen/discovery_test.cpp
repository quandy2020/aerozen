...
/// within the tests.
template <typename T>
class DiscoveryDerived : public Discovery<T>
{
    // Documentation inherited.
public:
    DiscoveryDerived(const std::string& _pUuid, const std::string& _ip,
    ...
    /// specified as a parameter. If false, we shouldn't have any activity
    /// stored.
public:
    void TestActivity(const std::string& _pUuid,
                      const bool _expectedActivity) const {
        EXPECT_EQ(this->activity.find(_pUuid) != this->activity.end(),
                  _expectedActivity);
    };
};
...
TEST(DiscoveryTest, TestAdvertise) {
    ...
    transport::Discovery<MessagePublisher> Discovery2(pUuid2, g_ip, g_msgPort,
                                                      true);
    ...
    MessagePublisher publisher2("/topic2", addr1, ctrl1, pUuid1, nUuid1, "type",
                                opts1);
...
TEST(DiscoveryTest, TestDiscover) {
    ...
    // from the Discover method this time because the topic information should
    // be known.
    EXPECT_TRUE(discovery2.Discover(g_topic));
...
TEST(DiscoveryTest, TestActivity) {
    ...
    {
        DiscoveryDerived<MessagePublisher> discovery2(proc2Uuid, g_ip,
                                                      g_msgPort);
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(discovery1.HeartbeatInterval() * 2));
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(discovery1.HeartbeatInterval()));
