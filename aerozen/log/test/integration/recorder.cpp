...
void VerifyMessage(const gz::transport::log::Message& _msg,
                   const int64_t _msgCount, const int64_t _numTopics,
...
TEST(recorder,
    ...
    const std::string logName =
        "file:recorderBeginRecordingTopicsBeforeAdvertise?mode=memory&cache="
        "shared";
    ...
    for (const gz::transport::log::Message& msg : log.QueryMessages()) {
        VerifyMessage(msg, count, static_cast<int64_t>(topics.size()),
                      VerifyTopic);
        ++count;
    }
...
TEST(recorder, BeginRecordingTopicsAfterAdvertisement) {
    ...
    const std::string logName =
        "file:recorderBeginRecordingTopicsAfterAdvertise?mode=memory&cache="
        "shared";
...
void RecordPatternBeforeAdvertisement(const std::regex& _pattern) {
    ...
    const std::string logName =
        "file:recorder_RecordPatternBeforeAdvertisement?mode=memory&cache="
        "shared";
...
TEST(recorder, DataWriterQueue) {
...

    {
        ...
        for (const auto& msg : log.QueryMessages()) {
            VerifyMessage(msg, count, 1, [&](const std::string& _topic) {
                return topic == _topic;
            });
            ++count;
        }
...
TEST(recorder, DataWriterQueueClockUpdates) {
...

    {
        ...
        for (const auto& msg : log.QueryMessages()) {
            ...
            EXPECT_EQ(expectedStamp.count(),
                      (msg.TimeReceived() - lastTs).count())
...
int main(int argc, char** argv) {
    ...
    gz::utils::setenv("GZ_TRANSPORT_LOG_SQL_PATH", GZ_TRANSPORT_LOG_SQL_PATH);
