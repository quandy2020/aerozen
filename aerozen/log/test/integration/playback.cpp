...
#include "aerozen/log/playback.hpp"
#include "aerozen/log/recorder.hpp"
 ...
 * SubscribeRaw.
 */
void TrackMessages(std::vector<MessageInformation>& _archive, const char* _data,
                   std::size_t _len,
                   const aerozen:MessageInfo& _msgInfo) {
 ...
 * @param[out] a boolean
 */
bool MessagesAreEqual(const MessageInformation& _recorded,
                      const MessageInformation& _played) {
    return _recorded.data == _played.data && _recorded.type == _played.type &&
 ...
 * @param[out] a boolean
 */
bool ExpectSameMessages(const std::vector<MessageInformation>& _recorded,
                        const std::vector<MessageInformation>& _played) {
    for (std::size_t i = 0; i < _recorded.size() && i < _played.size(); ++i) {
        if (!MessagesAreEqual(_recorded[i], _played[i]))
            return false;
    }
    if (_recorded.size() != _played.size())
        return false;
    return true;
}
...
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(ReplayLog)) {
    ...
    const std::string logName =
        "file:playbackReplayLog?mode=memory&cache=shared";
    EXPECT_EQ(aerozen::log::RecorderError::SUCCESS, recorder.Start(logName));

    const int numChirps = 100;
    auto chirper =
        aerozen::log::test::BeginChirps(topics, numChirps, partition);
    ...
    aerozen::log::Playback playback(logName);
    recorder.Stop();
    ...
    const std::chrono::milliseconds expectedDuration{
        numChirps * aerozen::log::test::DelayBetweenChirps_ms};
 ...
 * the original.
 */
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(ReplayNoSuchTopic)) {
    aerozen::log::Recorder recorder;
    const std::string logName =
        "file:playbackReplayNoSuchTopic?mode=memory&cache=shared";
    EXPECT_EQ(aerozen::log::RecorderError::SUCCESS,
 ...
 * the original.
 */
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(ReplayLogRegex)) {
    std::vector<std::string> topics = {"/foo", "/bar", "/baz"};
 ...
 */
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(RemoveTopic)) {
    std::vector<std::string> topics = {"/foo", "/bar", "/baz"};
    ...
    // Remove some topics without calling AddTopic(). This tells the Playback
    // that it should play all topics except for these.
...

    {
        ...
        // Wait to make sure our callbacks are done processing the incoming
        // messages (Strangely, Windows throws an exception when this is ~1s or
        // more)
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
 ...
 * the original.
 */
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(ReplayLogMoveInstances)) {
    std::vector<std::string> topics = {"/foo", "/bar", "/baz"};
    ...
    const int numChirps = 100;
    auto chirper = aerozen::log::test::BeginChirps(topics, numChirps, partition);
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(
        aerozen::log::test::DelayBetweenChirps_ms * numChirps / 2));
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(
        aerozen::log::test::DelayBetweenChirps_ms * numChirps / 4));
 ...
 */
TEST(playback, AEROZEN_UTILS_TEST_ENABLED_ONLY_ON_LINUX(ReplayStep)) {
    std::vector<std::string> topics = {"/foo", "/bar", "/baz"};
    ...
    std::chrono::milliseconds totalDurationMs(
        aerozen::log::test::DelayBetweenChirps_ms * numChirps);
 ...
 * the playback workflow.
 */
TEST(playback, AEROZEN_UTILS_TEST_DISABLED_ON_MAC(ReplaySeek)) {
    std::vector<std::string> topics = {"/foo", "/bar", "/baz"};
    ...
    handle->Seek(std::chrono::milliseconds(
        aerozen::log::test::DelayBetweenChirps_ms * 10));
    handle->Step(std::chrono::milliseconds(
        aerozen::log::test::DelayBetweenChirps_ms * 2));
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(
        aerozen::log::test::DelayBetweenChirps_ms * 5));
...
/**
 * @brief Main function for the playback test.
 */
int main(int argc, char** argv) {
    ...
    aerozen::utils::setenv("AEROZEN_LOG_SQL_PATH", AEROZEN_LOG_SQL_PATH);
    aerozen::utils::setenv(aerozen::log::SchemaLocationEnvVar,
                           AEROZEN_LOG_SQL_PATH);
