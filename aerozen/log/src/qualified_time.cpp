...
public:
    Implementation(const Time& _time, Qualifier _qualifier)
        : indeterminate(false), qualifier(_qualifier), time(_time) {
        // Do nothing
    }

    /// \internal \sa QualifiedTime()
public:
    Implementation() : indeterminate(true) {
        // Do nothing
    }
...
public:
    void SetTime(const std::chrono::nanoseconds& _time, Qualifier _qualifier) {
...
public:
    bool operator==(const Implementation& _other) const {
        // We mimic the behavior of NaN equality comparisons, which always
        // return false during an equality comparison.
...
//////////////////////////////////////////////////
QualifiedTime::QualifiedTime(const Time& _time, Qualifier _qualifier)
...
QualifiedTime::QualifiedTime()
    : dataPtr(new Implementation, [](Implementation* _impl) { delete _impl; }) {
    // Do nothing
}
...
//////////////////////////////////////////////////
void QualifiedTime::SetTime(const Time& _time, Qualifier _qualifier) {
...
public:
    Implementation(const QualifiedTime& _begin, const QualifiedTime& _end)
        : start(_begin), finish(_end) {
        // Do nothing
    }
...
public:
    QualifiedTime finish;
};

//////////////////////////////////////////////////
QualifiedTimeRange::QualifiedTimeRange(const QualifiedTime& _begin,
