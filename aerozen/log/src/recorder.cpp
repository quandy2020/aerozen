...
public:
    struct LogData {
        /// \brief Constructor
        LogData(std::chrono::nanoseconds _stamp,
                std::vector<char>&& _msgData,  // NOLINT
                const transport::MessageInfo& _msgInfo)
            : stamp(_stamp), msgData(std::move(_msgData)), msgInfo(_msgInfo) {}
        /// \brief Time stamp of when the message was received by the log
        /// recorder
...
public:
    int64_t AddTopic(const std::regex& _pattern);
...
public:
    void DecrementBufferSize(std::size_t _len);
...
public:
    void WriteToLogFile(const LogData& _logData);

    /// \brief log file or nullptr if not recording
public:
    std::unique_ptr<Log> logFile;
...
public:
    std::set<std::string> alreadySubscribed;
...
public:
    ...
    /// \brief Object for discovering new publishers as they advertise
    /// themselves
public:
    std::unique_ptr<MsgDiscovery> discovery;

    /// \brief Maximum size of the buffer (in bytes) that is used to store data
    /// from topic callbacks.
public:
    std::atomic<std::size_t> maxBufferSize{1000 << 20};

    /// \brief Current size of the buffer (in bytes). This is computed every
    /// time data is added or removed from the queue. Because of that, we'll use
...
public:
    ...
    /// overwritten. Thus, it is important to set the queue size appropriately
    /// for your application. The maximum size of this queue is determined by
    /// `maxBufferSize`. The current size of the buffer is calculated from
    /// `msgData`.
public:
    std::deque<LogData> dataQueue;
...
public:
    ...
    /// \brief Handle to worker thread that writes data from the dataQueue to
    /// the database
public:
...
public:
    std::atomic<bool> dataWriterState{false};
...
Recorder::Implementation::Implementation() {
    ...
    this->rawCallback = [this](const char* _data, std::size_t _len,
                               const transport::MessageInfo& _info) {
...
//////////////////////////////////////////////////
void Recorder::Implementation::OnMessageReceived(const char* _data,
                                                 std::size_t _len,
    ...
    // Don't store anything in the queue unless the data writer has started,
    // which happens when Recorder::Start is called.
    if (this->dataWriterState) {
        ...
        if (this->maxBufferSize > 0) {
            ...
            if ((this->bufferSize + _len > this->maxBufferSize) &&
                ...
                this->DecrementBufferSize(
                    this->dataQueue.front().msgData.size());
                this->dataQueue.pop_front();
            }
        }

        this->bufferSize += _len;
        // If the message being added here is larger than maxBufferSize, it
        // should still be recorded. It just means that the buffer cannot hold
        // another message until it is recorded.
        this->dataQueue.emplace_back(this->clock->Time(), std::move(tmp),
                                     _info);
...
void Recorder::Implementation::OnAdvertisement(const Publisher& _publisher) {
    ...
    TopicUtils::DecomposeFullyQualifiedTopic(_publisher.Topic(), partition,
                                             topic);
    ...
    // If the advertised partition does not match ours, ignore this
    // advertisement
...
void Recorder::Implementation::DataWriterThread() {
    while (this->dataWriterState) {
        ...
        if (this->dataQueue.empty()) {
            this->dataQueueCondVar.wait(lock, [this] {
...
void Recorder::Implementation::DecrementBufferSize(std::size_t _len) {
    if (this->bufferSize >= _len) {
        ...
        LERR(
            "Buffer size was decremented to a value less than zero. "
...
//////////////////////////////////////////////////
Recorder::Recorder() : dataPtr(new Implementation) {}
