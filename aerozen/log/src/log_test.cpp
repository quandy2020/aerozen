...
TEST(Log, InsertMessage) {
    ...
    EXPECT_TRUE(logFile.InsertMessage(
        0ns, "/some/topic/name", "some.message.type",
        reinterpret_cast<const void*>(data.c_str()), data.size()));
}
...
TEST(Log, InsertMessageGetMessages) {
    ...
    EXPECT_TRUE(logFile.InsertMessage(
        1s, "/some/topic/name", "some.message.type",
        reinterpret_cast<const void*>(data1.c_str()), data1.size()));

    EXPECT_TRUE(logFile.InsertMessage(
        2s, "/some/topic/name", "some.message.type",
        reinterpret_cast<const void*>(data2.c_str()), data2.size()));
...
TEST(Log, QueryMessagesByTopicNone) {
    ...
    auto batch = logFile.QueryMessages(log::TopicList::Create(noTopics));
...
TEST(Log, Insert2Get1MessageByTopic) {
    ...
    EXPECT_TRUE(logFile.InsertMessage(
        2s, "/second/topic/name", "some.message.type",
        reinterpret_cast<const void*>(data2.c_str()), data2.size()));

    {
        auto batch = logFile.QueryMessages(log::TopicList("/some/topic/name"));
...

    {
        ...
        auto batch = logFile.QueryMessages(log::TopicList(
            "/second/topic/name", log::QualifiedTimeRange(0s, 2s)));
        auto iter = batch.begin();
        ASSERT_NE(batch.end(), iter);
        EXPECT_EQ(data2, iter->Data());
        ++iter;
...
TEST(Log, CheckLogTimes) {
    ...
    EXPECT_TRUE(logFile.InsertMessage(
        1s, "/a/topic/name", "a.message.type",
        reinterpret_cast<const void*>(data1.c_str()), data1.size()));
    ...
    EXPECT_TRUE(logFile.InsertMessage(
        10s, "/another/topic/name", "another.message.type",
        reinterpret_cast<const void*>(data2.c_str()), data2.size()));
    EXPECT_EQ(1s, logFile.StartTime());
...
TEST(Log, OpenCorruptDatabase) {
    ...
    EXPECT_GT(logFile.EndTime(), 0ns)
        << "logFile.EndTime() == " << logFile.EndTime().count() << "ns";
    ;
}
