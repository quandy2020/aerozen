...
public:
    int EndTransactionIfEnoughTimeHasPassed();
...
public:
    ...
    /// \return one of the SQLite error codes
public:
    int BeginTransactionIfNotInOne();
...
public:
    int64_t InsertOrGetTopicId(const std::string& _name,
                               const std::string& _type);

    /// \brief Insert a message into the database
public:
    bool InsertMessage(const std::chrono::nanoseconds& _time, int64_t _topic,
...
public:
    bool TimeForNewTransaction() const;

    /// \brief SQLite3 database pointer wrapper
public:
    std::shared_ptr<raii_sqlite3::Database> db;
...
public:
    std::chrono::milliseconds transactionPeriod;

    /// \brief Flag to track whether we need to generate a new Descriptor
private:
    mutable bool needNewDescriptor = true;

    /// \brief Descriptor which provides insight to the column IDs in the
    /// database
public:
    mutable log::Descriptor descriptor;
...
public:
    std::chrono::nanoseconds startTime = std::chrono::nanoseconds(-1);

    /// \brief Time of the last message in the log file.
public:
    std::chrono::nanoseconds endTime = std::chrono::nanoseconds(-1);
};
    ...
    if (this->needNewDescriptor) {
        ...
        do {
            ...
            if (returnCode == SQLITE_ROW) {
                ...
                sqlite_int64 topicId =
                    sqlite3_column_int64(topicStatement.Handle(), 0);

                const unsigned char* topicName =
                    sqlite3_column_text(topicStatement.Handle(), 1);
                std::size_t lenTopicName =
                    sqlite3_column_bytes(topicStatement.Handle(), 1);

                const unsigned char* typeName =
                    sqlite3_column_text(topicStatement.Handle(), 2);
                std::size_t lenTypeName =
                    sqlite3_column_bytes(topicStatement.Handle(), 2);

                TopicKey key;
                key.topic = std::string(
                    reinterpret_cast<const char*>(topicName), lenTopicName);
                key.type = std::string(reinterpret_cast<const char*>(typeName),
                                       lenTypeName);
                topicsInLog[key] = topicId;
                LDBG(key.topic << "|" << key.type << "|" << topicId << "\n");
            } else if (returnCode != SQLITE_DONE) {
                LERR("Failed query topic ids: "
                     << sqlite3_errmsg(this->db->Handle()) << "\n");
                return nullptr;
            }
...
int Log::Implementation::EndTransaction() {
    // End the transaction
    int returnCode = sqlite3_exec(this->db->Handle(), "END;", NULL, 0, nullptr);
...
int Log::Implementation::BeginTransactionIfNotInOne() {
    ...
    int returnCode =
        sqlite3_exec(this->db->Handle(), "BEGIN;", NULL, 0, nullptr);
    if (returnCode != SQLITE_OK) {
...
//////////////////////////////////////////////////
int64_t Log::Implementation::InsertOrGetTopicId(const std::string& _name,
                                                const std::string& _type) {
    ...
    raii_sqlite3::Statement messageTypeStatement(*(this->db), sqlMessageType);
    if (!messageTypeStatement) {
        LERR("Failed to compile statement to insert message type\n");
        return -1;
    }
    raii_sqlite3::Statement topicStatement(*(this->db), sqlTopic);
    if (!topicStatement) {
    ...
    int returnCode;
    // Bind parameters
    returnCode = sqlite3_bind_text(messageTypeStatement.Handle(), 1,
                                   _type.c_str(), _type.size(), nullptr);
    if (returnCode != SQLITE_OK) {
        LERR("Failed to bind message type name(1): " << returnCode << "\n");
        return -1;
    }
    returnCode = sqlite3_bind_text(topicStatement.Handle(), 1, _type.c_str(),
                                   _type.size(), nullptr);
    if (returnCode != SQLITE_OK) {
        LERR("Failed to bind message type name(2): " << returnCode << "\n");
        return -1;
    }
    returnCode = sqlite3_bind_text(topicStatement.Handle(), 2, _name.c_str(),
                                   _name.size(), nullptr);
    if (returnCode != SQLITE_OK) {
...
//////////////////////////////////////////////////
bool Log::Implementation::InsertMessage(const std::chrono::nanoseconds& _time,
                                        const int64_t _topic, const void* _data,
    ...
    if (returnCode != SQLITE_DONE) {
        LERR("Failed to insert message. sqlite3 return code["
             << returnCode << "] data[" << _data << "] len[" << _len << "]\n");
        return false;
    }
    return true;
}
...
//////////////////////////////////////////////////
Log::Log() : dataPtr(new Implementation) {
...
bool Log::Open(const std::string& _file, const std::ios_base::openmode _mode) {
    ...
    if (!*(db)) {
        // The constructor of raii_sqlite3::Database will print out the reason
        // that the database failed to open.
        return false;
    }
    ...
    if (std::ios_base::out & _mode) {
        ...
        if (!fin) {
            LERR("Failed to open schema [" << schemaFile << "].\n"
                                           << " Set " << SchemaLocationEnvVar
                                           << " to the schema location.\n");
            return false;
        }
        ...
        int returnCode =
            sqlite3_exec(db->Handle(), schema.c_str(), NULL, 0, NULL);
        if (returnCode != SQLITE_OK) {
...
//////////////////////////////////////////////////
bool Log::InsertMessage(const std::chrono::nanoseconds& _time,
    ...
    if (SQLITE_OK != this->dataPtr->EndTransactionIfEnoughTimeHasPassed()) {
        ...
        LERR("Failed to end transaction: "
             << sqlite3_errmsg(this->dataPtr->db->Handle()) << "\n");
        return false;
    }
...
Batch Log::QueryMessages(const QueryOptions& _options) {
    ...
    std::unique_ptr<BatchPrivate> batchPriv(new BatchPrivate(
        this->dataPtr->db, _options.GenerateStatements(*desc)));
...
std::chrono::nanoseconds Log::EndTime() const {
    ...
    if (resultCode == SQLITE_CORRUPT) {
        LERR(
            "Database is corrupt, retrieving last valid message."
