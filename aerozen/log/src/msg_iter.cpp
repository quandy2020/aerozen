...
bool MsgIterPrivate::PrepareNextStatement() {
    ...
    if (!*nextStatement) {
        LERR("Failed to prepare query: " << sqlite3_errmsg(this->db->Handle())
                                         << "\n");
        return false;
    }
        ...
        switch (param.Type()) {
            case SqlParameter::ParamType::TEXT:
                returnCode = sqlite3_bind_text(
                    nextStatement->Handle(), i, param.QueryText()->c_str(),
                    param.QueryText()->size(), SQLITE_STATIC);
                break;
...
void MsgIterPrivate::StepStatement() {
    if (this->statement) {
        ...
        if (returnCode == SQLITE_ROW) {
            // TODO(anyone) get data and create message in the dereference
            // operators Assumes statement has column order: messages id (0),
            // timeRecv(1), topics name(2), message_type name(3), message
            // data(4)
            std::chrono::nanoseconds timeRecv;

            // Time received
            sqlite_int64 timeRecvInt =
                sqlite3_column_int64(this->statement->Handle(), 1);
            timeRecv = std::chrono::nanoseconds(timeRecvInt);

            // Topic name
            const unsigned char* topic =
                sqlite3_column_text(this->statement->Handle(), 2);
            std::size_t numTopic =
                sqlite3_column_bytes(this->statement->Handle(), 2);

            // Message type name
            const unsigned char* type =
                sqlite3_column_text(this->statement->Handle(), 3);
            std::size_t numType =
                sqlite3_column_bytes(this->statement->Handle(), 3);

            // Message data
            const void* data =
                sqlite3_column_blob(this->statement->Handle(), 4);
            std::size_t numData =
                sqlite3_column_bytes(this->statement->Handle(), 4);

            this->message.reset(new Message(
                timeRecv, data, numData, reinterpret_cast<const char*>(type),
                numType, reinterpret_cast<const char*>(topic), numTopic));
        } else {
...
//////////////////////////////////////////////////
MsgIter::MsgIter() : dataPtr(new MsgIterPrivate) {}
...
bool MsgIter::operator==(const MsgIter& _other) const {
    // TODO(anyone) this won't work once this class has a proper copy
    // constructor It's only good enough to compare this with an empty iterator
