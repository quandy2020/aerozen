...
public:
    Implementation(const std::string& _file, const NodeOptions& _nodeOptions)
    ...
    /// \brief This gets used by RemoveTopic(~) to make sure we follow the
    /// correct behavior.
    void DefaultToAllTopics() {
        if (!this->addTopicWasUsed) {
            const Descriptor* desc = this->logFile->Descriptor();
            const Descriptor::NameToMap& allTopics =
                desc->TopicsToMsgTypesToId();
...
public:
    std::weak_ptr<PlaybackHandle> lastHandle;
...
public:
    Implementation(const std::shared_ptr<Log>& _logFile,
                   const std::unordered_set<std::string>& _topics,
                   const std::chrono::nanoseconds& _waitAfterAdvertising,
                   const NodeOptions& _nodeOptions, bool _msgWaiting);
...
public:
    void CreatePublisher(const std::string& _topic, const std::string& _type);
...
public:
    bool WaitUntil(const std::chrono::nanoseconds& _targetTime);
...
public:
    ...
    /// \note This member needs to come before the publishers member so that
    /// they get destructed in the correct order
public:
...
public:
    std::unordered_map<
        std::string,
        std::unordered_map<std::string, gz::transport::Node::Publisher>>
        publishers;
...
public:
    std::condition_variable stopConditionVariable;

    /// \brief True if the thread should be stopped
public:
    std::atomic_bool stop;

    /// \brief This will be switched to true as the playback thread exits. This
    /// is different from `stop` because it cannot be changed by the user; it
    /// only changes exactly when the playback thread exits.
public:
...
public:
    std::condition_variable pauseConditionVariable;
...
public:
    const std::unordered_set<std::string> trackedTopics;
...
public:
    Batch::iterator messageIter;

    // \brief The wall clock time of the first message in batch
public:
    std::chrono::nanoseconds firstMessageTime;
...
PlaybackHandlePtr Playback::Start(
    ...
    if (!kSqlite3Threadsafe) {
        ...
        if (lastHandle && !lastHandle->Finished()) {
            LWRN(
                "You have linked to a single-threaded sqlite3. We can only "
                "spawn "
    ...
    PlaybackHandlePtr newHandle(
        new PlaybackHandle(std::make_unique<PlaybackHandle::Implementation>(
            this->dataPtr->logFile, topics, _waitAfterAdvertising,
            this->dataPtr->nodeOptions, _msgWaiting)));

    // We only need to store this if sqlite3 was not compiled in threadsafe
    // mode.
    if (!kSqlite3Threadsafe)
...
//////////////////////////////////////////////////
void PlaybackHandle::Implementation::AddTopic(const std::string& _topic) {
    const Descriptor* desc = this->logFile->Descriptor();
    const Descriptor::NameToMap& allTopics = desc->TopicsToMsgTypesToId();
...
//////////////////////////////////////////////////
void PlaybackHandle::Implementation::CreatePublisher(const std::string& _topic,
                                                     const std::string& _type) {
    auto firstMapIter = this->publishers.find(_topic);
    if (firstMapIter == this->publishers.end()) {
        // Create a map for the message topic
        this->publishers[_topic] =
            std::unordered_map<std::string, Node::Publisher>();
...
void PlaybackHandle::Implementation::WaitUntilFinished() {
    if (this->logFile->Valid() && !this->stop) {
        ...
        this->waitConditionVariable.wait(
            lk, [this] { return this->finished.load(); });
    }
}
...
void PlaybackHandle::Implementation::StartPlayback() {
    ...
    // Set time boundary to infinite, which will get overwritten on a step
    // request
    ...
    this->playbackThread = std::thread([this]() mutable {
        while (!this->stop && (this->messageIter != this->batch.end())) {
            ...
            if (this->paused) {
                ...
                this->pauseConditionVariable.wait(
                    lk, [this] { return !this->paused.load(); });
                this->lastEventTime =
                    std::chrono::steady_clock::now().time_since_epoch();
                // Abort current iteration after coming back from pause
                continue;
            }
            // If not executing a requested step (regular non-paused playback
            // flow)
            if (this->nextMessageTime <= this->boundaryTime) {
                // The timeDelta becomes the time remaining until next message
                const std::chrono::nanoseconds timeDelta(this->nextMessageTime -
                                                         this->playbackTime);
                const std::chrono::nanoseconds timeToWaitUntil(
                    this->lastEventTime + timeDelta);
                // Wait until target time is reached or playback is
                // stopped/paused In the latter case, break the iteration step
                ...
                // Publish the message
                {
                    ...
                    LDBG("publishing\n");
                    this->publishers[this->messageIter->Topic()]
                                    [this->messageIter->Type()]
                                        .PublishRaw(this->messageIter->Data(),
                                                    this->messageIter->Type());
            ...
            // then goes back to paused.
                ...
                // The timeDelta is equal to the step size passed to the step
                // function
                const std::chrono::nanoseconds timeDelta(this->boundaryTime -
                                                         this->playbackTime);
...
void PlaybackHandle::Implementation::Pause() {
    ...
    if (!this->paused) {
        ...
        this->playbackTime = this->playbackTime + now - this->lastEventTime;
