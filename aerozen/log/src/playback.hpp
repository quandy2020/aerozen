...
public:
    ...
    /// sqlite3 database in multiple threads (see
    /// https://www.sqlite.org/threadsafe.html). In most cases there should be
    /// no issue, but if you or a library you are using calls sqlite3_config(~)
    /// to change the threading mode to Single-thread or Multi-thread (instead
    /// of the default setting of Serialized), then starting multiple
    ...
    /// nullptr.
public:
    [[nodiscard]] PlaybackHandlePtr Start(
...
private:
    PlaybackHandle(std::unique_ptr<Implementation>&& _internal);  // NOLINT
