...
#pragma warning(pop)
    ...
    inline bool operator==(const TopicKey& _other) const {
        return (this->topic == _other.topic && this->type == _other.type);
    }
};
...
public:
    void Reset(const TopicKeyMap& _topics);
...
public:
    NameToMap topicsToMsgTypesToId;

    /// \internal \sa Descriptor::MsgTypesToTopicsToId()
public:
    NameToMap msgTypesToTopicsToId;
#ifdef _WIN32
#pragma warning(pop)
#endif
};
...
namespace std {
template <>
struct hash<gz::transport::log::TopicKey> {
    size_t operator()(const gz::transport::log::TopicKey& _key) const {
