...
TEST(QualifiedTimeRange, AllTime) {
    ...
    EXPECT_TRUE(
        log::QualifiedTimeRange::AllTime().Beginning().IsIndeterminate());
...
TEST(QualifiedTimeRange, EqualityOperators) {
    ...
    EXPECT_TRUE(log::QualifiedTimeRange(qt1, qt2) ==
                log::QualifiedTimeRange(qt1, qt2));
    EXPECT_FALSE(log::QualifiedTimeRange(qt1, qt2) !=
                 log::QualifiedTimeRange(qt1, qt2));

    EXPECT_FALSE(log::QualifiedTimeRange(qt1, qt2) ==
                 log::QualifiedTimeRange(qt2, qt1));
    EXPECT_TRUE(log::QualifiedTimeRange(qt1, qt2) !=
                log::QualifiedTimeRange(qt2, qt1));

    EXPECT_FALSE(log::QualifiedTimeRange(qt1, qt2) ==
                 log::QualifiedTimeRange(qt2, qt2));
    EXPECT_TRUE(log::QualifiedTimeRange(qt1, qt2) !=
                log::QualifiedTimeRange(qt2, qt2));
}
