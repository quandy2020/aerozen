...
Database::Database(const std::string& _path, int _flags) {
    // Open the database;
    int return_code =
        sqlite3_open_v2(_path.c_str(), &(this->handle), _flags, nullptr);
...
Statement::Statement(Database& _db, const std::string& _sql) {
    int return_code = sqlite3_prepare_v2(_db.Handle(), _sql.c_str(),
                                         _sql.size(), &(this->handle), NULL);

    if (return_code != SQLITE_OK) {
        if (this->handle) {
            LERR("Failed to prepare statement" << sqlite3_errmsg(_db.Handle())
                                               << "\n");
