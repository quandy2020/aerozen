...
static void AppendTopicListClause(SqlStatement& _sql,
                                  const std::vector<int64_t>& _ids) {
...
public:
    SqlStatement GenerateTimeConditions() const {
        ...
        if (start.IsIndeterminate() && finish.IsIndeterminate()) {
            // We return a blank SQL statement, because we have no time
            // constraints.
            return SqlStatement();
        }
        ...
        if (!start.IsIndeterminate()) {
            if (*start.GetQualifier() == QualifiedTime::Qualifier::INCLUSIVE)
                startCompare = ">=";
            else if (*start.GetQualifier() ==
                     QualifiedTime::Qualifier::EXCLUSIVE)
                startCompare = ">";
        }
        ...
        if (!startCompare.empty()) {
            ...
            sql.parameters.emplace_back(
                static_cast<int64_t>(start.GetTime()->count()));
        ...
        if (!finishCompare.empty()) {
            ...
            sql.parameters.emplace_back(
                static_cast<int64_t>(finish.GetTime()->count()));
        }
...
{
    /// \brief Generate a WHERE clause for topics that exist in the requested
    /// list
    /// \param[in] _descriptor The descriptor forwarded by the interface class
    /// \return The desired WHERE clause
public:
    SqlStatement GenerateStatement(const Descriptor& _descriptor) {
...
//////////////////////////////////////////////////
TopicList::TopicList(const std::set<std::string>& _topics,
                     const QualifiedTimeRange& _timeRange)
    : TimeRangeOption(_timeRange), dataPtr(new Implementation{_topics}) {
    // Do nothing
}

//////////////////////////////////////////////////
TopicList::TopicList(const std::string& _singleTopic,
                     const QualifiedTimeRange& _timeRange)
...
TopicList::TopicList(const TopicList& _other)
    : TimeRangeOption(_other), dataPtr(new Implementation{*_other.dataPtr}) {
    // Do nothing
}
...
{
    /// \brief Generate a WHERE clause for topics that match the requested
    /// pattern
    /// \param[in] _descriptor The descriptor forwarded by the interface class
    /// \return The desired WHERE clause
public:
...
//////////////////////////////////////////////////
TopicPattern::TopicPattern(const std::regex& _pattern,
                           const QualifiedTimeRange& _timeRange)
    : TimeRangeOption(_timeRange), dataPtr(new Implementation{_pattern}) {
    // Do nothing
}
...
TopicPattern::TopicPattern(TopicPattern&& _other)  // NOLINT(build/c++11)
    : TimeRangeOption(std::move(_other)), dataPtr(std::move(_other.dataPtr)) {
    // Do nothing
}
...
AllTopics::AllTopics(const QualifiedTimeRange& _timeRange)
    ...
    // Do nothing. We don't even allocate the PIMPL because we have no use for
    // it yet. We might never need it, but we'll hang onto it just in case the
    // needs of this class change in the future.
}

//////////////////////////////////////////////////
AllTopics::AllTopics(const AllTopics& _other) : TimeRangeOption(_other) {
    // Do nothing
}
