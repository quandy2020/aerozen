...
#ifndef AEROZEN_UTILS__SUBPROCESS_HPP_
#define AEROZEN_UTILS__SUBPROCESS_HPP_

#include "aerozen/utils/detail/subprocess.hpp"
...
public:
    explicit Subprocess(const std::vector<std::string>& _commandLine)
        : commandLine(_commandLine), environment({}), inheritEnvironment(true) {
        this->Create();
    }
...
public:
    Subprocess(const std::vector<std::string>& _commandLine,
               aerozen::utils::EnvironmentMap _environment)
...
public:
    Subprocess(const std::vector<std::string>& _commandLine,
               const aerozen::utils::EnvironmentStrings& _environment)
        : Subprocess(_commandLine,
                     aerozen::utils::envStringsToMap(_environment)) {}
...
private:
    void Create() {
        ...
        auto environmentStr =
            aerozen::utils::envMapToStrings(this->environment);
        ...
        int ret = -1;
        if (!this->inheritEnvironment) {
            ret = subprocess_create_ex(commandLineCstr.data(), 0,
                                       environmentCstr.data(), this->process);
        } else {
...
protected:
    aerozen::utils::EnvironmentMap environment;
...
#endif  // AEROZEN_UTILS__SUBPROCESS_HPP_
