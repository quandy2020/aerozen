...
#ifndef AEROZEN_UTILS__DETAIL__IMPLPTR_HPP
#define AEROZEN_UTILS__DETAIL__IMPLPTR_HPP

#include "aerozen/utils/impl_ptr.hpp"
...
CopyMoveDeleteOperations<T, CopyConstruct,
                         CopyAssign>::CopyMoveDeleteOperations(C&& _construct,
                                                               A&& _assign)
    : construct(_construct), assign(_assign) {
    // Do nothing
}
}  // namespace detail
...
ImplPtr<T, Deleter, Operations>::ImplPtr(Ptr* _ptr, D&& _deleter, Ops&& _ops)
    : ptr(_ptr, std::forward<D>(_deleter)), ops(std::forward<Ops>(_ops)) {
    // Do nothing
}
...
//////////////////////////////////////////////////
template <class T, class Deleter, class Operations>
auto ImplPtr<T, Deleter, Operations>::operator=(const ImplPtr& _other)
    -> ImplPtr& {
    if (this->ptr)
...
ImplPtr<T> MakeImpl(Args&&... _args) {
    return ImplPtr<T>(
        new T{std::forward<Args>(_args)...}, &detail::DefaultDelete<T>,
        detail::CopyMoveDeleteOperations<T>(&detail::DefaultCopyConstruct<T>,
...
UniqueImplPtr<T> MakeUniqueImpl(Args&&... args) {
    return UniqueImplPtr<T>(new T{std::forward<Args>(args)...},
                            &detail::DefaultDelete<T>);
}
}  // namespace utils
}  // namespace aerozen

#endif  // AEROZEN_UTILS__DETAIL__IMPLPTR_HPP
