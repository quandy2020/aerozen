...
TEST(ClockTest, WallClock) {
    ...
    EXPECT_GE(endTime - startTime, expectedSleepTime)
        << "Expected[" << (endTime - startTime).count() << "] Actual["
        << sleepTime.count() << "]\n";
}
...
using TimeBase = NetworkClock::TimeBase;

/// \brief A test fixture for NetworkClock related tests,
/// parameterized by NetworkClock::TimeBase.
...
protected:
    msgs::Clock MakeClockMessage(const std::chrono::seconds& _secs,
...
TEST_P(NetworkClockTest, Functionality) {
    ...
    NetworkClock clock(clockTopicName, GetParam());
...
INSTANTIATE_TEST_SUITE_P(TestAllTimeBases, NetworkClockTest,
                         ::testing::Values(TimeBase::SIM, TimeBase::REAL,
                                           TimeBase::SYS));

/// \brief Check NetworkClock functionality.
TEST(ClockTest, BadNetworkClock) {
    const std::string badClockTopicName{"//bad-clock"};
    const NetworkClock badTopicClock(badClockTopicName);
    EXPECT_FALSE(badTopicClock.IsReady());
    const NetworkClock::TimeBase badTimebase =
        static_cast<NetworkClock::TimeBase>(-1);
    const std::string clockTopicName{"/clock"};
