...
public:
    ...
    bool WaitUntil(Lock& _lock, const unsigned int _timeout) {
        ...
        return this->condition.wait_until(
            _lock, now + std::chrono::milliseconds(_timeout),
            [this] { return this->repAvailable; });
    }
...
public:
    explicit ReqHandler(const std::string& _nUuid) : IReqHandler(_nUuid) {}
     ...
     */
    void SetCallback(
        const std::function<void(const Rep& _rep, const bool _result)>& _cb) {
        this->cb = _cb;
    }
    ...
    void SetMessage(const Req* _reqMsg) {
        if (!_reqMsg) {
            std::cerr << "ReqHandler::SetMessage() _reqMsg is null"
                      << std::endl;
            return;
        }
...
private:
     ...
     * false otherwise.
     */
    std::function<void(const Rep& _rep, const bool _result)> cb;
};
...
public:
    ...
    void SetResponse(const google::protobuf::Message* _repMsg) {
        if (!_repMsg) {
            std::cerr << "ReqHandler::SetResponse() _repMsg is null"
                      << std::endl;
            return;
        }
...
private:
     ...
     */
    std::unique_ptr<google::protobuf::Message> repMsg;
};
}  // namespace aerozen
