...
public:
    explicit CaptureStreamScoped(std::ostream& stream)
        : streamToCapture{stream}, originalCerrStreamBuf{stream.rdbuf()} {
...
{
public:
    CaptureCerrScoped() : CaptureStreamScoped(std::cerr) {}
};

class CaptureCoutScoped : public CaptureStreamScoped
{
public:
    CaptureCoutScoped() : CaptureStreamScoped(std::cout) {}
};
...
TEST_F(ParametersClientTest, SetParameter) {
    ...
    }
    {
        CaptureCoutScoped coutCapture;
        CaptureCerrScoped cerrCapture;
        cmdParameterSet("", "parameter2", "gz_msgs.StringMsg",
                        "data: \"foobar\"");
        auto output = coutCapture.str();
        EXPECT_NE(std::string::npos, output.find("successfully"));
        EXPECT_EQ(cerrCapture.str(), "");
    }
    {
        CaptureCoutScoped coutCapture;
        CaptureCerrScoped cerrCapture;
        cmdParameterSet("/ns", "another_param1", "gz_msgs.Boolean",
                        "data: true");
        auto output = coutCapture.str();
        EXPECT_NE(std::string::npos, output.find("successfully"));
        EXPECT_EQ(cerrCapture.str(), "");
    ...
    }
    {
        ...
        EXPECT_NE(std::string::npos,
                  output.find("string representation is invalid"));
    }
    {
        CaptureCerrScoped cerrCapture;
