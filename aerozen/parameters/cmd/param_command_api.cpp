...
#include "aerozen/parameters/cmd/param_command_api.hpp"
...
extern "C" void CmdParametersList(const char* _ns) {
    ...
    std::cout << std::endl
              << "Listing parameters, registry namespace [" << _ns << "]..."
              << std::endl
              << std::endl;
    try {
...
extern "C" void CmdParameterSet(const char* _ns, const char* _paramName,
    ...
    std::cout << std::endl
              << "Setting parameter [" << _paramName
              << "] for registry namespace [" << _ns << "]..." << std::endl;

    auto msg = aerozen::Factory::New(_paramType);
    if (!msg) {
        std::cerr << "Could not create a message of type [" << _paramType
                  << "]. The message type is invalid." << std::endl;
        return;
    }
    if (!google::protobuf::TextFormat::ParseFromString(_paramValue,
                                                       msg.get())) {
        std::cerr << "Could not create a message of type [" << _paramType
