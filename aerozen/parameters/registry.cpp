...
#include "aerozen/parameters/registry.hpp"
...
#include "aerozen/proto/boolean.pb.h"
#include "aerozen/proto/parameter.pb.h"
#include "aerozen/proto/parameter_declarations.pb.h"
#include "aerozen/proto/parameter_error.pb.h"
#include "aerozen/proto/parameter_name.pb.h"
#include "aerozen/proto/parameter_value.pb.h"

#include "aerozen/parameters/result.hpp"
#include "aerozen/parameters/utils.hpp"

namespace aerozen::parameters {
struct ParametersRegistryPrivate {
    using ParametersMapT =
    ...
    bool GetParameter(const aerozen::msgs::ParameterName& _req,
                      aerozen::msgs::ParameterValue& _res);

    /// \brief List parameter service callback.
    /// \param[in] _req unused.
    /// \param[out] _res List of available parameters.
    /// \return True if successful.
    bool ListParameters(const aerozen::msgs::Empty& _req,
    ...
    /// \param[out] _res Unused.
    /// \return True if successful.
    bool DeclareParameter(const msgs::Parameter& _req,
                          msgs::ParameterError& _res);
...
ParametersRegistry::ParametersRegistry(
    ...
    std::string getParameterSrvName{_parametersServicesNamespace +
                                    "/get_parameter"};
    this->dataPtr->node.Advertise(getParameterSrvName,
                                  &ParametersRegistryPrivate::GetParameter,
                                  this->dataPtr.get());

    std::string listParametersSrvName{_parametersServicesNamespace +
                                      "/list_parameters"};
    this->dataPtr->node.Advertise(listParametersSrvName,
                                  &ParametersRegistryPrivate::ListParameters,
                                  this->dataPtr.get());

    std::string setParameterSrvName{_parametersServicesNamespace +
                                    "/set_parameter"};
    this->dataPtr->node.Advertise(setParameterSrvName,
                                  &ParametersRegistryPrivate::SetParameter,
                                  this->dataPtr.get());

    std::string declareParameterSrvName{_parametersServicesNamespace +
                                        "/declare_parameter"};
    this->dataPtr->node.Advertise(declareParameterSrvName,
                                  &ParametersRegistryPrivate::DeclareParameter,
                                  this->dataPtr.get());
}
...
ParametersRegistry& ParametersRegistry::operator=(ParametersRegistry&&) =
    default;
...
bool ParametersRegistryPrivate::ListParameters(
    const msgs::Empty&, msgs::ParameterDeclarations& _res) {
...
bool ParametersRegistryPrivate::SetParameter(const msgs::Parameter& _req,
                                             msgs::ParameterError& _res) {
    (void)_res;
    const auto& paramName = _req.name();
    {
        ...
        auto requestedGzTypeOpt = getGzTypeFromAnyProto(_req.value());
...
bool ParametersRegistryPrivate::DeclareParameter(const msgs::Parameter& _req,
                                                 msgs::ParameterError& _res) {
    (void)_res;
    ...
    auto paramValue = aerozen::msgs::Factory::New(gzType);
    if (!paramValue) {
        _res.set_data(msgs::ParameterError::INVALID_TYPE);
        return true;
    }
...
ParameterResult ParametersRegistry::DeclareParameter(
    const std::string& _parameterName,
    std::unique_ptr<google::protobuf::Message> _initialValue) {
    if (!_initialValue) {
        throw std::invalid_argument{
            "ParametersRegistry::DeclareParameter(): `_parameterName` is "
            "nullptr"};
    }
    ...
    if (!it_emplaced_pair.second) {
        return ParameterResult{ParameterResultType::AlreadyDeclared,
                               _parameterName};
    }
...
ParameterResult ParametersRegistry::DeclareParameter(
    ...
    auto newParam = aerozen::msgs::Factory::New(protoType);
    if (!newParam) {
...
ParameterResult ParametersRegistry::Parameter(
    ...
    if (newProtoType != protoType) {
        return ParameterResult{ParameterResultType::InvalidType, _parameterName,
...
ParameterResult ParametersRegistry::Parameter(
    ...
    const auto& protoType = it->second->GetDescriptor()->full_name();
    _parameter = aerozen::msgs::Factory::New(std::string(protoType));
    if (!_parameter) {
        return ParameterResult{ParameterResultType::InvalidType, _parameterName,
                               std::string(protoType)};
    }
...
ParameterResult ParametersRegistry::SetParameter(
    const std::string& _parameterName,
    ...
    if (it->second->GetDescriptor() != _value->GetDescriptor()) {
        return ParameterResult{
            ParameterResultType::InvalidType, _parameterName,
...
ParameterResult ParametersRegistry::SetParameter(
    ...
    // Validate the type matches before copying.
    if (it->second->GetDescriptor() != _value.GetDescriptor()) {
        return ParameterResult{ParameterResultType::InvalidType,
                               _parameterName};
    }
    it->second->CopyFrom(_value);
    return ParameterResult{ParameterResultType::Success};
}
aerozen::msgs::ParameterDeclarations ParametersRegistry::ListParameters()
    const {
    aerozen::msgs::ParameterDeclarations ret;
    aerozen::msgs::Empty unused;
