...
#include "aerozen/parameters/client.hpp"
...
using namespace aerozen::parameters;

class ParametersClientTest : public ::testing::Test
{
protected:
    void SetUp() override {
        registry_.DeclareParameter("parameter1",
                                   std::make_unique<msgs::Boolean>());
        registry_.DeclareParameter("parameter2",
        ...
        registry_other_ns_.DeclareParameter("another_param1",
                                            std::make_unique<msgs::Boolean>());
        anotherStrMsg = std::make_unique<msgs::StringMsg>();
        anotherStrMsg->set_data("bsd");
        registry_other_ns_.DeclareParameter("another_param2",
                                            std::move(anotherStrMsg));
    }
...
TEST_F(ParametersClientTest, SetParameter) {
    ...
    }
    {
        ...
        EXPECT_FALSE(result) << result;
        EXPECT_EQ(result.ResultType(), ParameterResultType::NotDeclared)
            << result;
    ...
    }
    {
        ...
        {
            auto result =
                registry_other_ns_.Parameter("another_param1", msg_got);
            EXPECT_TRUE(result) << result;
        }
...
TEST_F(ParametersClientTest, DeclareParameter) {
    ...
    }
    {
        ...
        {
            auto result =
                registry_other_ns_.Parameter("new_parameter", msg_got);
            EXPECT_TRUE(result) << result;
        }
        EXPECT_EQ(msg_got.data(), true);
    }
...
TEST_F(ParametersClientTest, ListParameters) {
    {
        ...
        for (auto decl : declarations.parameter_declarations()) {
            if (decl.name() == "parameter1" &&
                decl.type() == "gz.msgs.Boolean") {
                foundParam1 = true;
            }
            if (decl.name() == "parameter2" &&
                decl.type() == "gz.msgs.StringMsg") {
                foundParam2 = true;
            }
            if (decl.name() == "parameter3" &&
                decl.type() == "gz.msgs.StringMsg") {
                foundParam3 = true;
            }
        }
        EXPECT_TRUE(foundParam1)
            << "expected to find declaration for parameter1";
        EXPECT_TRUE(foundParam2)
            << "expected to find declaration for parameter2";
        EXPECT_TRUE(foundParam3)
            << "expected to find declaration for parameter3";
    }
    {
        ParametersClient client{"/ns"};
        auto declarations = client.ListParameters();
        ...
        bool foundParam2{false};
        for (auto decl : declarations.parameter_declarations()) {
            if (decl.name() == "another_param1" &&
                decl.type() == "gz.msgs.Boolean") {
                foundParam1 = true;
            }
            if (decl.name() == "another_param2" &&
                decl.type() == "gz.msgs.StringMsg") {
                foundParam2 = true;
            }
        }
