...
TEST(ParametersRegistry, DeclareParameter) {
    ParametersRegistry registry{""};
    EXPECT_THROW(registry.DeclareParameter("will_fail", nullptr),
                 std::invalid_argument);
    registry.DeclareParameter("parameter1",
                              std::make_unique<aerozen::msgs::Boolean>());
    auto ret = registry.DeclareParameter(
        "parameter1", std::make_unique<aerozen::msgs::Boolean>());
    EXPECT_FALSE(ret);
...
TEST(ParametersRegistry, SetParameter) {
    ...
    auto ret = registry.SetParameter(
        "will_fail", std::make_unique<aerozen::msgs::Boolean>());
    EXPECT_FALSE(ret);
    EXPECT_EQ(ret.ResultType(), ParameterResultType::NotDeclared);
    EXPECT_EQ(ret.ParamName(), "will_fail");
    registry.DeclareParameter("parameter1",
    ...
    ret = registry.SetParameter("parameter1",
                                std::make_unique<aerozen::msgs::StringMsg>());
    EXPECT_FALSE(ret);
    EXPECT_EQ(ret.ResultType(), ParameterResultType::InvalidType);
    EXPECT_EQ(ret.ParamName(), "parameter1");
    EXPECT_EQ(ret.ParamType(), "aerozen.msgs.Boolean");
...
TEST(ParametersRegistry, ListParameters) {
    ...
    registry.DeclareParameter("parameter1",
                              std::make_unique<aerozen::msgs::Boolean>());
    registry.DeclareParameter("parameter2",
                              std::make_unique<aerozen::msgs::StringMsg>());
    ...
    for (auto decl : declarations.parameter_declarations()) {
        if (decl.name() == "parameter1" &&
            decl.type() == "aerozen.msgs.Boolean") {
            foundParam1 = true;
        }
        if (decl.name() == "parameter2" &&
            decl.type() == "aerozen.msgs.StringMsg") {
            foundParam2 = true;
        }
    }
