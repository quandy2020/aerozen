...
/// \class Discovery discovery.hpp aerozen/discovery.hpp
...
public:
    Discovery(const std::string& _pUuid, const std::string& _ip,
        ...
        if (bind(this->sockets.at(0), reinterpret_cast<sockaddr*>(&localAddr),
...
public:
    ...
    //////////////////////////////////////////////////
public:
    void LivelinessMsgDataHandler(const zenoh::Sample& _sample) {
        ...
        std::string msgType;
        if (!TopicUtils::DecomposeLivelinessToken(token, prefix, partition,
                                                  topic, pUUID, nUUID,
                                                  entityType, msgType)) {
            return;
        };

        MessagePublisher pub("@" + partition + "@" + topic, "", this->pUuid,
                             pUUID, nUUID, msgType, AdvertiseMessageOptions());

        {
            ...
            if (_sample.get_kind() == Z_SAMPLE_KIND_PUT) {
                if (entityType == "MP") {
                    ...
                    // TODO(azeey) Should not call this callbacks while we have
                    // the mutex locked.
                ...
                if (entityType == "MP") {
                    this->info.DelPublisherByNode(pub.Topic(), pub.PUuid(),
                                                  pub.NUuid());
                } else if (entityType == "MS" && this->pUuid != pUUID) {
    ...
    //////////////////////////////////////////////////
public:
    void LivelinessSrvDataHandler(const zenoh::Sample& _sample) {
        DiscoveryCallback<Pub> cb;
        std::string token{_sample.get_keyexpr().as_string_view()};
        std::string prefix;
        std::string partition;
        ...
        ServicePublisher pub("@" + partition + "@" + service, "", "", pUUID,
                             nUUID, reqType, repType,
...
public:
    void Start(std::shared_ptr<zenoh::Session> _session,
        ...
        if (found) {
            ...
            for (const auto& proc : addresses) {
                for (const auto& node : proc.second) {
                    if (cb) {
                        // Execute the user's callback for a service request.
                        // Notice that we only execute one callback for
                        // preventing receive
...
public:
    void SendSubscribersRep(const MessagePublisher& _pub) const {
        this->SendMsg(DestinationType::ALL, msgs::Discovery::SUBSCRIBERS_REP,
                      _pub);
    }
...
public:
    void Register(const MessagePublisher& _pub) const {
        this->SendMsg(DestinationType::ALL, msgs::Discovery::NEW_CONNECTION,
                      _pub);
    }
...
public:
    void Unregister(const MessagePublisher& _pub) const {
        this->SendMsg(DestinationType::ALL, msgs::Discovery::END_CONNECTION,
                      _pub);
    }
...
public:
    bool Unadvertise(const std::string& _topic, const std::string& _nUuid) {
        Pub inf;
        {
            ...
            // Don't do anything if the topic is not advertised by any of my
            // nodes
        ...
        if (inf.Options().Scope() != Scope_t::PROCESS) {
            this->SendMsg(DestinationType::ALL, msgs::Discovery::UNADVERTISE,
                          inf);
        }
...
public:
    void PrintCurrentState() const {
        ...
        std::cout << "---------------" << std::endl;
        std::cout << std::boolalpha << "Enabled: " << this->enabled
                  << std::endl;
        std::cout << "Discovery state" << std::endl;
        std::cout << "\tUUID: " << this->pUuid << std::endl;
        std::cout << "Settings" << std::endl;
        std::cout << "\tActivity: " << this->activityInterval << " ms."
                  << std::endl;
        std::cout << "\tHeartbeat: " << this->heartbeatInterval << "ms."
                  << std::endl;
        std::cout << "\tSilence: " << this->silenceInterval << " ms."
                  << std::endl;
...
public:
    void TopicList(std::vector<std::string>& _topics) {
        ...
        this->SendMsg(DestinationType::ALL, msgs::Discovery::SUBSCRIBERS_REQ,
                      pub);
...

        {
            ...
            if (!this->initialized) {
                if (this->numHeartbeatsUninitialized == 2u) {
                    // We consider discovery initialized after two heartbeat
                    // cycles.
                    this->initialized = true;

                    // Notify anyone waiting for the initialization phase to
                    // finish.
...
private:
    int NextTimeout() const {
        ...
        int t = static_cast<int>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::min(timeUntilNextHeartbeat, timeUntilNextActivity))
                .count());
...
private:
    void RecvDiscoveryUpdate() {
        ...
        int64_t received = recvfrom(
            this->sockets.at(0), reinterpret_cast<raw_type*>(rcvStr),
            this->kMaxRcvStr, 0, reinterpret_cast<sockaddr*>(&clntAddr),
...
private:
    void DispatchDiscoveryMsg(const std::string& _fromIp, char* _msg,
        ...
        bool isSenderLocal =
            (std::find(this->hostInterfaces.begin(), this->hostInterfaces.end(),
                       _fromIp) != this->hostInterfaces.end()) ||
            (_fromIp.find("127.") == 0);
        ...
        switch (msg.type()) {
            ...
            case msgs::Discovery::SUBSCRIBE: {
                ...
                // Check if at least one of my nodes advertises the topic
                // requested.
                Addresses_M<Pub> addresses;
                {
                    std::lock_guard<std::mutex> lock(this->mutex);
...
private:
    void SendUnicast(const msgs::Discovery& _msg) const {
        uint16_t msgSize;
        ...
        if (_msg.SerializeToArray(buffer + sizeof(msgSize), msgSize)) {
            ...
            for (const auto& sockAddr : this->relayAddrs) {
                ...
                auto sent = sendto(
                    ...
                    totalSize, 0, reinterpret_cast<const sockaddr*>(&sockAddr),
                    sizeof(sockAddr));
    ...
    /// \param[in] _msg Discovery message.
private:
    void SendMulticast(const msgs::Discovery& _msg) const {
        uint16_t msgSize;
...
private:
    const sockaddr_in* MulticastAddr() const {
...
private:
    bool RegisterNetIface(const std::string& _ip) {
...
private:
    static const uint16_t kMaxRcvStr = std::numeric_limits<uint16_t>::max();
...
private:
    std::vector<sockaddr_in> relayAddrs;
...
private:
    std::unique_ptr<zenoh::Subscriber<void>> livelinessSubscriber;
#endif
