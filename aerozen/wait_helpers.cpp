...
/**
 * @brief Platform shims for pipe / read / write. On Windows the C runtime
 * provides POSIX-compatible _pipe / _read / _write in <io.h>.
 * @param[in] _fds File descriptors.
...
bool WaitForService(const Node& _node, const std::string& _service,
...
bool WaitForTopic(const Node& _node, const std::string& _topic,
                  std::chrono::milliseconds _timeout) {
    return WaitUntil(
        [&]() {
            std::vector<std::string> topics;
            _node.TopicList(topics);
            return std::find(topics.begin(), topics.end(), _topic) !=
                   topics.end();
        },
        _timeout, std::chrono::milliseconds(100));
}
