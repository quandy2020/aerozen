...
bool Node::Subscriber::Unsubscribe() {
    ...
    bool res = this->dataPtr->shared->Unsubscribe(
        this->dataPtr->topic, this->dataPtr->nUuid, this->dataPtr->nOpts,
        this->dataPtr->hUuid);
...
bool Node::Publisher::Publish(const ProtoMsg& _msg) {
    ...
    const std::string& publisherMsgType =
    ...
    if (publisherMsgType != _msg.GetTypeName()) {
        std::cerr << "Node::Publisher::Publish() Type mismatch.\n"
                  << "\t* Type advertised: "
                  << this->dataPtr->publisher.MsgTypeName()
                  << "\n\t* Type published: " << _msg.GetTypeName()
                  << std::endl;
        return false;
    }
    ...
    const NodeShared::SubscriberInfo& subscribers =
        this->dataPtr->shared->CheckSubscriberInfo(publisherTopic,
                                                   publisherMsgType);
    ...
    if (subscribers.haveRaw || subscribers.haveRemote) {
        ...
        // want to send a corrupt/bad message to some subscribers and not
        // others.
    ...
    if (subscribers.haveLocal || subscribers.haveRaw) {
        ...
        // out of scope.
        pubMsgDetails->info.SetTopicAndPartition(
                ...
                for (const auto& handler : node.second) {
                    if (!handler.second) {
                        std::cerr << "Node::Publisher::Publish(): "
                                  << "NULL local subscription handler"
                                  << std::endl;
                        continue;
                    }
        ...
        if (subscribers.haveRaw) {
            for (auto& node : subscribers.rawHandlers) {
                for (auto& handler : node.second) {
                    const RawSubscriptionHandlerPtr& rawHandler =
                        handler.second;

                    if (!rawHandler) {
                        std::cerr << "Node::Publisher::Publish(): "
                                  << "NULL raw subscription handler"
                                  << std::endl;
                        continue;
                    }
                    ...
                    if (!pubMsgDetails->sharedBuffer) {
                        ...
                        memcpy(pubMsgDetails->sharedBuffer.get(), msgBuffer,
                               msgSize);
                    }
    ...
    if (impl == "zeromq") {
        if (subscribers.haveRemote) {
            ...
            if (!this->dataPtr->shared->Publish(
                    this->dataPtr->publisher.Topic(), msgBuffer, msgSize,
                    myDeallocator, std::string(_msg.GetTypeName()))) {
                return false;
            }
        } else {
            delete[] msgBuffer;
...
std::vector<std::string> Node::SubscribedTopics() const {
    ...
    // I'm a real subscriber if I have interest in a topic and I know a
    // publisher.
...
void Node::TopicList(std::vector<std::string>& _topics) const {
    ...
    for (const auto& fullyQualifiedTopic : allTopics) {
        ...
        std::string topic;
        TopicUtils::DecomposeFullyQualifiedTopic(fullyQualifiedTopic, partition,
                                                 topic);
...
void Node::ServiceList(std::vector<std::string>& _services) const {
    ...
    for (auto& service : allServices) {
        ...
        std::string partition =
            service.substr(1, service.find_last_of("@") - 1);
        // Remove the front '/'
        if (!partition.empty())
            partition.erase(partition.begin());
...
bool Node::SubscribeRaw(const std::string& _topic, const RawCallback& _callback,
    ...
    if (impl == "zeromq") {
        ...
        handlerPtr->SetCallback(std::move(_callback), this->Shared()->Session(),
                                fullyQualifiedTopic);
    }
#endif
    else
...
bool Node::EnableStats(const std::string& _topic, bool _enable,
    ...
    this->dataPtr->statPub =
        this->Advertise(_publicationTopic, "gz.msgs.Metric", opts);
    ...
    this->dataPtr->shared->EnableStats(fullyQualifiedTopic, _enable, statCb);
...
bool Node::ServiceInfo(const std::string& _service,
    ...
    std::string fullyQualifiedTopic;
    if (!TopicUtils::FullyQualifiedName(this->Options().Partition(),
                                        this->Options().NameSpace(), _service,
                                        fullyQualifiedTopic)) {
        return false;
    }
    ...
    // Copy the publishers.
    for (SrvAddresses_M::iterator iter = pubs.begin(); iter != pubs.end();
         ++iter) {
        for (std::vector<ServicePublisher>::iterator pubIter =
                 iter->second.begin();
             pubIter != iter->second.end(); ++pubIter) {
            // Add the publisher if it doesn't already exist.
...
Node::Publisher Node::Advertise(const std::string& _topic,
    ...
    if (std::find(currentTopics.begin(), currentTopics.end(), _topic) !=
        currentTopics.end()) {
    ...
    MessagePublisher publisher(fullyQualifiedTopic, this->Shared()->MyAddress(),
                               // this->Shared()->myControlAddress,
                               "unused", this->Shared()->pUuid,
                               this->NodeUuid(), _msgTypeName, _options);

    std::string impl = this->dataPtr->shared->GzImplementation();
    if (impl == "zeromq") {
        if (!this->Shared()->dataPtr->msgDiscovery->Advertise(publisher)) {
            std::cerr << "Node::Advertise(): Error advertising topic [" << topic
