...
#include "aerozen/test/test_config.hpp"
#include "aerozen/utils/environment.hpp"

using ZenohConfigSource = aerozen::NodeSharedPrivate::ZenohConfigSource;

TEST(ZenohConfigTest, LoadFromEnvVariable) {
    // Set ZENOH_CONFIG to point to our test config file
    ASSERT_TRUE(aerozen::utils::setenv("ZENOH_CONFIG", kZenohTestConfig));

    aerozen::NodeSharedPrivate nodePrivate;
    ...
    // Clean up
    ASSERT_TRUE(aerozen::utils::unsetenv("ZENOH_CONFIG"));
}

TEST(ZenohConfigTest, LoadDefaultConfig) {
    // Ensure ZENOH_CONFIG is not set
    aerozen::utils::unsetenv("ZENOH_CONFIG");
...
TEST(ZenohConfigTest, NonexistentConfigPath) {
    ...
    ASSERT_TRUE(aerozen::utils::setenv("ZENOH_CONFIG",
...
TEST(ZenohConfigTest, InvalidConfigContent) {
    ...
    ASSERT_TRUE(aerozen::utils::setenv("ZENOH_CONFIG", kZenohInvalidConfig));
...
TEST(ZenohConfigTest, ConfigOverrideSingle) {
    ...
    aerozen::NodeSharedPrivate::ApplyZenohConfigOverrides(
...
TEST(ZenohConfigTest, ConfigOverrideEmpty) {
    ...
    aerozen::NodeSharedPrivate::ApplyZenohConfigOverrides(config, "");
...
TEST(ZenohConfigTest, ConfigOverrideViaEnvVar) {
    ASSERT_TRUE(aerozen::utils::setenv("AEROZEN_ZENOH_CONFIG_OVERRIDE",
                                       "scouting/multicast/enabled=false"));
    ...
    const char* overrideEnv = std::getenv("AEROZEN_ZENOH_CONFIG_OVERRIDE");
    ASSERT_NE(nullptr, overrideEnv);
    aerozen::NodeSharedPrivate::ApplyZenohConfigOverrides(config, overrideEnv);

    EXPECT_EQ("false", config.get("scouting/multicast/enabled"));

    ASSERT_TRUE(aerozen::utils::unsetenv("AEROZEN_ZENOH_CONFIG_OVERRIDE"));
}
