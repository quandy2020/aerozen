...
TEST(ZenohConfigTest, NonexistentConfigPath) {
    ...
    ASSERT_TRUE(
        gz::utils::setenv("ZENOH_CONFIG", "/nonexistent/path/to/config.json5"));
...
TEST(ZenohConfigTest, ConfigOverrideViaEnvVar) {
    ASSERT_TRUE(gz::utils::setenv("GZ_TRANSPORT_ZENOH_CONFIG_OVERRIDE",
                                  "scouting/multicast/enabled=false"));
    ...
    const char* overrideEnv = std::getenv("GZ_TRANSPORT_ZENOH_CONFIG_OVERRIDE");
