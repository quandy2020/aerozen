...
#ifndef AEROZEN_DETAIL_NODE_HPP_
#define AEROZEN_DETAIL_NODE_HPP_
...
Node::Publisher Node::Advertise(const std::string& _topic,
    ...
    return this->Advertise(_topic, std::string(MessageT().GetTypeName()),
                           _options);
}
...
Node::Subscriber Node::CreateSubscriber(const std::string& _topic,
                                        Args&&... args) {
    auto handler = this->SubscribeImpl(_topic, std::forward<Args>(args)...);
    if (handler && !handler->HandlerUuid().empty()) {
        return Node::Subscriber(_topic, handler->NodeUuid(), this->Options(),
                                handler->HandlerUuid());
    }
...
template <typename MessageT>
std::shared_ptr<SubscriptionHandler<MessageT>> Node::SubscribeImpl(
    const std::string& _topic, std::function<void(const MessageT& _msg)> _cb,
    const SubscribeOptions& _opts) {
    std::function<void(const MessageT&, const MessageInfo&)> f =
...
std::shared_ptr<SubscriptionHandler<MessageT>> Node::SubscribeImpl(
    const std::string& _topic, void (ClassT::*_cb)(const MessageT& _msg),
    ClassT* _obj, const SubscribeOptions& _opts) {
    std::function<void(const MessageT&, const MessageInfo&)> f =
...
std::shared_ptr<SubscriptionHandler<MessageT>> Node::SubscribeImpl(
    ...
    const SubscribeOptions& _opts) {
    std::function<void(const MessageT&, const MessageInfo&)> f =
        [_cb](const MessageT& _internalMsg, const MessageInfo& _internalInfo) {
...
std::shared_ptr<SubscriptionHandler<MessageT>> Node::SubscribeImpl(
    ...
    FullyQualifiedTopic fullyQualifiedTopic(this->Options().Partition(),
                                            this->Options().NameSpace(), topic);
    ...
    std::shared_ptr<SubscriptionHandler<MessageT>> subscrHandlerPtr(
        new SubscriptionHandler<MessageT>(this->Shared()->pUuid,
                                          this->NodeUuid(), _opts));
    ...
    if (impl == "zeromq") {
        ...
        subscrHandlerPtr->SetCallback(std::move(_cb), this->Shared()->Session(),
                                      fullyQualifiedTopic);
    }
#endif
    else
...
bool Node::Advertise(const std::string& _topic, bool (*_cb)(ReplyT& _reply),
                     const AdvertiseServiceOptions& _options) {
...
bool Node::Advertise(const std::string& _topic,
                     ...
                     const AdvertiseServiceOptions& _options) {
    std::function<bool(const RequestT&, msgs::Empty&)> f =
        [_cb](const RequestT& _internalReq, msgs::Empty& /*_internalRep*/) {
            (*_cb)(_internalReq);
            return true;
        };
...
bool Node::Advertise(const std::string& _topic,
    ...
    if (impl == "zeromq")
        ...
        repHandlerPtr->SetCallback(_cb, this->Shared()->Session(),
                                   fullyQualifiedTopic);
    }
#endif
    ...
    this->Shared()->Repliers().AddHandler(fullyQualifiedTopic, this->NodeUuid(),
                                          repHandlerPtr);

    if (impl == "zeromq") {
        // Notify the discovery service to register and advertise my responser.
        ServicePublisher publisher(
            fullyQualifiedTopic, this->Shared()->ReplierAddress(),
            this->Shared()->replierId.ToString(), this->Shared()->pUuid,
            this->NodeUuid(), std::string(RequestT().GetTypeName()),
...
bool Node::Advertise(const std::string& _topic,
                     bool (ClassT::*_cb)(const RequestT& _request,
                                         ReplyT& _reply),
                     ClassT* _obj, const AdvertiseServiceOptions& _options) {
    std::function<bool(const RequestT&, ReplyT&)> f =
        [_cb, _obj](const RequestT& _internalReq, ReplyT& _internalRep) {
...
bool Node::Advertise(const std::string& _topic,
                     bool (ClassT::*_cb)(ReplyT& _reply), ClassT* _obj,
                     const AdvertiseServiceOptions& _options) {
    std::function<bool(const msgs::Empty&, ReplyT&)> f =
...
//////////////////////////////////////////////////
template <typename RequestT, typename ReplyT>
bool Node::Request(const std::string& _topic, const RequestT& _request,
...
//////////////////////////////////////////////////
template <typename ReplyT>
bool Node::Request(const std::string& _topic,
                   void (*_cb)(const ReplyT& _reply, const bool _result)) {
    msgs::Empty req;
...
template <typename RequestT, typename ReplyT>
bool Node::Request(
    const std::string& _topic, const RequestT& _request,
    ...
    IRepHandlerPtr repHandler;
    {
        ...
        localResponserFound = this->Shared()->Repliers().FirstHandler(
            fullyQualifiedTopic, std::string(RequestT().GetTypeName()),
            std::string(ReplyT().GetTypeName()), repHandler);
    }
...
template <typename ClassT, typename RequestT, typename ReplyT>
bool Node::Request(const std::string& _topic, const RequestT& _request,
                   void (ClassT::*_cb)(const ReplyT& _reply,
                                       const bool _result),
                   ClassT* _obj) {
    std::function<void(const ReplyT&, const bool)> f =
...
template <typename RequestT, typename ReplyT>
bool Node::Request(const std::string& _topic, const RequestT& _request,
                   const unsigned int& _timeout, ReplyT& _reply,
                   bool& _result) {
    // Topic remapping.
    std::string topic = _topic;
    this->Options().TopicRemap(_topic, topic);
    ...
    // Store the request handler.
    this->Shared()->Requests().AddHandler(fullyQualifiedTopic, this->NodeUuid(),
                                          reqHandlerPtr);
    ...
    if (this->Shared()->TopicPublishers(fullyQualifiedTopic, addresses)) {
        ...
        // Discover the service responser.
        if (!this->Shared()->DiscoverService(fullyQualifiedTopic)) {
            std::cerr << "Node::Request(): Error discovering service [" << topic
                      << "]. Did you forget to start the discovery service?"
                      << std::endl;
            return false;
        }
    ...
    if (!_reply.ParseFromString(reqHandlerPtr->Response())) {
        std::cerr << "Node::Request(): Error Parsing the response" << std::endl;
        _result = false;
        return true;
    }
...
//////////////////////////////////////////////////
template <typename ReplyT>
bool Node::Request(const std::string& _topic, const unsigned int& _timeout,
...
bool Node::Request(const std::string& _topic, const RequestT& _request) {
    ...
    std::function<void(const msgs::Empty&, const bool)> f =
        [](const msgs::Empty&, const bool) {};

    return this->Request<RequestT, msgs::Empty>(_topic, _request, f);
}
}  // namespace aerozen

#endif  // AEROZEN_DETAIL_NODE_HPP_