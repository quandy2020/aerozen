...
```{.sh}
mkdir ~/aerozen_tutorial
cd ~/aerozen_tutorial
```

## Publisher

Copy `example/publisher.cpp` from this repository into the `aerozen_tutorial`
folder and open it with your favorite editor:

\snippet example/publisher.cpp complete
...
The line `#include "aerozen/node.hpp"` contains the core transport APIs used by
aerozen nodes.
...
Copy `example/subscriber.cpp` into the `aerozen_tutorial` folder and open it
with your favorite editor:

\snippet example/subscriber.cpp complete
...
Copy `example/CMakeLists.txt` from this repository into the `aerozen_tutorial`
folder.

Once you have all your files, go ahead and create a `build/` directory within
the `aerozen_tutorial` directory.
...
**NOTE:**
It is essential to have a valid value of `AEROZEN_PARTITION` environment
variable and to have it set to the same value in all open terminals. As
`AEROZEN_PARTITION`
...
in `AEROZEN_PARTITION`. aerozen prints error `Invalid partition name` in such
case. To resolve that, set `AEROZEN_PARTITION` explicitly to a valid value:
```bash
# Linux and Mac
export AEROZEN_PARTITION=test
# Windows
set AEROZEN_PARTITION=test
```
...
```{.cpp}
  ...
  aerozen::AdvertiseMessageOptions opts;
  opts.SetMsgsPerSec(1u);

  auto pub = node.Advertise<aerozen::msgs::StringMsg>(topic, opts);
  if (!pub)
  {
    std::cerr << "Error advertising topic [" << topic << "]" << std::endl;
    return -1;
...
`aerozen::msgs::StringMsg`. As the name of this section suggests, it is also
...
between aerozen and another protocol or if you want to just print the
content of a generic protobuf message using
`google::protobuf::TextFormat::PrintToString()`, among other use cases.

Copy `example/subscriber_generic.cpp` into the `aerozen_tutorial` folder and
...
`aerozen::MessageInfo &_info` provides some information about the
...
We use aerozen message types in most of our examples and tests. This decision
was made just for convenience but aerozen also supports the direct use of
Protobuf
...
Copy `example/publisher_custom_msg.cpp`, `example/subscriber_custom_msg.cpp`,
`example/msgs/stringmsg.proto`, `example/msgs/CMakeLists.txt`, and
`example/CMakeLists.txt` from this repository into your tutorial workspace.

### Walkthrough

There's nothing new to show in the `publisher_custom_msg.cpp` or
`subscriber_custom_msg.cpp` besides the use of your custom Protobuf message.
The only relevant parts are in the `CMakeLists.txt`
files.
...
```{.cpp}
if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/publisher_custom_msg.cpp")
  add_executable(publisher_custom_msg
    publisher_custom_msg.cpp
    ${PROTO_SRC})
  target_link_libraries(publisher_custom_msg
     ${AEROZEN_EXAMPLE_CORE_TARGET}
  )
  add_dependencies(publisher_custom_msg protobuf_compilation)
endif()

if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/subscriber_custom_msg.cpp")
  add_executable(subscriber_custom_msg
    subscriber_custom_msg.cpp
    ${PROTO_SRC})
...
set(PROTO_FILE "${PROTO_DIR}/stringmsg.proto")
set(PROTO_SRC "${PROTO_DIR}/stringmsg.pb.cc")
set(PROTO_HEADER "${PROTO_DIR}/stringmsg.pb.h")
...
add_custom_command(
  ...
  DEPENDS "${PROTO_FILE}"
  COMMENT "Generating custom protobuf messages"
  VERBATIM
)
...
```{.cpp}
  ...
  aerozen::NodeOptions nodeOptions;
  nodeOptions.AddTopicRemap("/foo", "/bar");
  aerozen::Node node(nodeOptions);
  std::string topic = "/foo";
```
...
You can also verify topic remapping by modifying the subscriber to use `/bar`
or by running `example/subscriber_generic.cpp` to inspect received topic names.
