...
#       call aerozen_pkg_check_modules_quiet(~).
#
...
#       aerozen_pkg_check_modules as follows:
#
#       aerozen_pkg_check_modules(SomePackage "libsomepackage >= 3.2.1")
#
...
#
#       aerozen_pkg_check_modules(SomePackage libsomepackage)
#
#       Without the version comparison, the quotes and spacing are irrelevant.
#       This usage note applies to aerozen_pkg_check_modules_quiet(~) as well.
#
macro(aerozen_pkg_check_modules package signature)

  aerozen_pkg_check_modules_quiet(${package} "${signature}" ${ARGN})
...
# This is an alternative to aerozen_pkg_check_modules(~) which you can use if you
...
#
# For usage instructions, see aerozen_pkg_check_modules(~) above.
macro(aerozen_pkg_check_modules_quiet package signature)
  ...
  #------------------------------------
  # Parse the arguments
  _aerozen_cmake_parse_arguments(aerozen_pkg_check_modules "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(aerozen_pkg_check_modules_INTERFACE)
    set(_aerozen_pkg_check_modules_interface_option INTERFACE)
  else()
    set(_aerozen_pkg_check_modules_interface_option) # Intentionally blank
  endif()

  if(NOT aerozen_pkg_check_modules_TARGET_NAME)
    set(aerozen_pkg_check_modules_TARGET_NAME "${package}::${package}")
  endif()

  find_package(PkgConfig QUIET)

  aerozen_pkg_config_entry(${package} "${signature}")

  if(PKG_CONFIG_FOUND)

    if(${aerozen_pkg_check_modules_NO_CMAKE_ENVIRONMENT_PATH})
      set(aerozen_pkg_check_modules_no_cmake_environment_path_arg
          NO_CMAKE_ENVIRONMENT_PATH)
    else()
      set(aerozen_pkg_check_modules_no_cmake_environment_path_arg)
    endif()

    if(${aerozen_pkg_check_modules_QUIET} OR ${package}_FIND_QUIETLY)
      set(aerozen_pkg_check_modules_quiet_arg QUIET)
    else()
      set(aerozen_pkg_check_modules_quiet_arg)
    endif()
    ...
    pkg_check_modules(${package}
                      ${aerozen_pkg_check_modules_quiet_arg}
                      ${aerozen_pkg_check_modules_no_cmake_environment_path_arg}
                      ${signature})
    ...
    if(${package}_FOUND)
      ...
      #
      # TODO(duyongquan): Consider giving a different prefix (e.g. AEROZEN_PC_${package})
      ...
      # to the aerozen_create_imported_target macro, so we will change the variable to
      ...
      _aerozen_pkgconfig_find_libraries(
        ${package}_LIBRARIES
        ${package}
        "${${package}_LIBRARIES}"
        "${${package}_LIBRARY_DIRS}")

      include(AerozenImportTarget)
      aerozen_import_target(${package} ${_aerozen_pkg_check_modules_interface_option}
        TARGET_NAME ${aerozen_pkg_check_modules_TARGET_NAME})
...
# This creates variables which inform aerozen_find_package(~) that your package
# should be found as a module by pkg-config. In most cases, this will be called
# implicitly by aerozen_pkg_check_modules[_quiet], but if a package provides both a
...
macro(aerozen_pkg_config_entry package string)
...
# This creates variables which inform aerozen_find_package(~) that your package must
...
# <package>.pc file.
macro(aerozen_pkg_config_library_entry package lib_name)
...
function(_aerozen_pkgconfig_find_libraries output_var package library_names library_dirs)
