...
# -------------------
#
# aerozen_configure_build()
#
# Configures the build rules of a Aerozen library project.
#
#===============================================================================
...
#################################################
# Configure the build of the Aerozen project
...
macro(aerozen_configure_build)
  ...
  cmake_parse_arguments(aerozen_configure_build "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(aerozen_configure_build_HIDE_SYMBOLS_BY_DEFAULT)
  ...
  if(build_warnings)
    ...
    foreach (msg ${build_warnings})
      aerozen_string_append(all_warnings " -- ${msg}" DELIM "\n")
    endforeach ()
  ...
  if(build_errors)
    ...
    if(aerozen_configure_build_QUIT_IF_BUILD_ERRORS)
  ...
  if(NOT build_errors)
    ...
    # CPPCHECK_DIRS is used in AerozenCodeCheck. The variable specifies the
    ...
    if(EXISTS "${CMAKE_SOURCE_DIR}/include")
      set(aerozen_doxygen_component_input_dirs "${CMAKE_SOURCE_DIR}/include")
    else()
      set(aerozen_doxygen_component_input_dirs "")
    endif()

    #--------------------------------------
    # Add the source code directories of each component if they exist
    foreach(component ${aerozen_configure_build_COMPONENTS})
      ...
      if(NOT SKIP_${component} AND NOT INTERNAL_SKIP_${component})
        ...
        if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${component}/include")
          ...
          # dependent on the implementation of aerozen_string_append, so be careful
          # when changing the implementation of that function.
          aerozen_string_append(aerozen_doxygen_component_input_dirs
            "${CMAKE_CURRENT_LIST_DIR}/${component}/include"
            DELIM " \\\\\\\\\n  ")
        endif()
        ...
        if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${component}/CMakeLists.txt")
          ...
          _aerozen_find_include_script(COMPONENT ${component})
        ...
        if(SKIP_${component})
          aerozen_string_append(skip_msg "by user request")
        elseif(${component}_MISSING_DEPS)
          aerozen_string_append(skip_msg "because the following packages are missing: ${${component}_MISSING_DEPS}")
        endif()
    ...
    _aerozen_export_target_all()

    #--------------------------------------
    # Create codecheck target
    include(AerozenCodeCheck)
    _aerozen_setup_target_for_codecheck()
...
macro(_aerozen_set_cxx_feature_flags)

  set(AEROZEN_KNOWN_CXX_STANDARDS 11 14 17)
  set(AEROZEN_CXX_11_FEATURES cxx_std_11)
  set(AEROZEN_CXX_14_FEATURES cxx_std_14)
  set(AEROZEN_CXX_17_FEATURES cxx_std_17)

endmacro()

function(_aerozen_find_include_script)
  ...
  #------------------------------------
  # Parse the arguments
  cmake_parse_arguments(_aerozen_find_include_script "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  #------------------------------------
  # Set the starting point
  set(include_start "${CMAKE_CURRENT_LIST_DIR}")

  if(_aerozen_find_include_script_COMPONENT)
    aerozen_string_append(include_start ${_aerozen_find_include_script_COMPONENT} DELIM "/")
  endif()
  ...
  if(EXISTS "${include_start}/include")
    ...
    elseif(EXISTS "${include_start}/include/aerozen/CMakeLists.txt")
      add_subdirectory("${include_start}/include/aerozen")
...
macro(_aerozen_parse_build_type)
  ...
  elseif("${CMAKE_BUILD_TYPE_UPPERCASE}" STREQUAL "COVERAGE")
    include(AerozenCodeCoverage)
    set(BUILD_TYPE_DEBUG TRUE)
    aerozen_setup_target_for_coverage(
  ...
  elseif("${CMAKE_BUILD_TYPE_UPPERCASE}" STREQUAL "PROFILE")
    ...
    aerozen_build_error("CMAKE_BUILD_TYPE [${CMAKE_BUILD_TYPE}] unknown. Valid options are: Debug Release RelWithDebInfo MinSizeRel Profile Check")
  endif()
