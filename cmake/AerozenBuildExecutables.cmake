...
#################################################
macro(aerozen_install_executable _name)
  set_target_properties(${_name} PROPERTIES VERSION ${PROJECT_VERSION_FULL})
  install (TARGETS ${_name} DESTINATION ${AEROZEN_BIN_INSTALL_DIR})
  manpage(${_name} 1)
endmacro()

#################################################
macro(aerozen_add_executable _name)
  add_executable(${_name} ${ARGN})
  target_link_libraries(${_name} ${general_libraries})
endmacro()

#################################################
# aerozen_build_executables(SOURCES <sources>
#                       [PREFIX <prefix>]
...
#                       [EXCLUDE_PROJECT_LIB])
#
# Build executables for an aerozen project. Arguments are as follows:
#
...
#
macro(aerozen_build_executables)
  # Define the expected arguments
  set(options EXCLUDE_PROJECT_LIB)
  set(oneValueArgs PREFIX EXEC_LIST)
  set(multiValueArgs SOURCES LIB_DEPS INCLUDE_DIRS)

  if(aerozen_build_executables_EXEC_LIST)
    set(${aerozen_build_executables_EXEC_LIST} "")
  endif()
  #------------------------------------
  # Parse the arguments
  _aerozen_cmake_parse_arguments(aerozen_build_executables "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  foreach(exec_file ${aerozen_build_executables_SOURCES})

    get_filename_component(BINARY_NAME ${exec_file} NAME_WE)
    set(BINARY_NAME ${aerozen_build_executables_PREFIX}${BINARY_NAME})

    add_executable(${BINARY_NAME} ${exec_file})

    if(aerozen_build_executables_EXEC_LIST)
      list(APPEND ${aerozen_build_executables_EXEC_LIST} ${BINARY_NAME})
    endif()

    if(NOT aerozen_build_executables_EXCLUDE_PROJECT_LIB)
      target_link_libraries(${BINARY_NAME} ${PROJECT_LIBRARY_TARGET_NAME})
    endif()

    if(aerozen_build_executables_LIB_DEPS)
      target_link_libraries(${BINARY_NAME} ${aerozen_build_executables_LIB_DEPS})
    endif()
    ...
    target_include_directories(${BINARY_NAME}
      PRIVATE
        ${PROJECT_SOURCE_DIR}
        ${PROJECT_BINARY_DIR}
        ${aerozen_build_executables_INCLUDE_DIRS})
