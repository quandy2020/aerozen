...
#################################################
# aerozen_target_interface_include_directories(<target> [include_targets])
#
...
function(aerozen_target_interface_include_directories name)
...
#################################################
macro(aerozen_install_includes _subdir)
  install(FILES ${ARGN}
    DESTINATION ${AEROZEN_INCLUDE_INSTALL_DIR}/${_subdir} COMPONENT headers)
endmacro()



#################################################
# aerozen_install_all_headers(
...
# ${AEROZEN_DESIGNATION}.hh. This will only add them to the header, it will not
# generate or install them.
#
# This will also run configure_file on aerozen_auto_headers.hh.in and config.hh.in
...
#
function(aerozen_install_all_headers)
  ...
  #------------------------------------
  # Parse the arguments
  _aerozen_cmake_parse_arguments(aerozen_install_all_headers "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  ...
  foreach(f ${all_files})
    ...
    if(IS_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/${f})
      ...
      list(FIND aerozen_install_all_headers_EXCLUDE_DIRS ${f} f_index)

      set(append_file TRUE)
      foreach(subdir ${aerozen_install_all_headers_EXCLUDE_DIRS})
  ...
  foreach(dir ${directories})
    ...
    if(headers)
      foreach(exclude ${aerozen_install_all_headers_EXCLUDE_FILES})
    ...
    foreach(header ${headers})
      set(aerozen_headers "${aerozen_headers}#include <${PROJECT_INCLUDE_DIR}/${header}>\n")
    endforeach()

    if("." STREQUAL ${dir})
      set(destination "${AEROZEN_INCLUDE_INSTALL_DIR_FULL}/${PROJECT_INCLUDE_DIR}")
    else()
      set(destination "${AEROZEN_INCLUDE_INSTALL_DIR_FULL}/${PROJECT_INCLUDE_DIR}/${dir}")
    endif()
  ...
  foreach(header ${aerozen_install_all_headers_GENERATED_HEADERS})
      set(aerozen_headers "${aerozen_headers}#include <${PROJECT_INCLUDE_DIR}/${header}>\n")
  endforeach()

  if(aerozen_install_all_headers_COMPONENT)

    set(component_name ${aerozen_install_all_headers_COMPONENT})

    # Define the install directory for the "config" header
    # The "meta" header will be installed one folder above this
    set(config_header_install_dir ${AEROZEN_INCLUDE_INSTALL_DIR_FULL}/${PROJECT_INCLUDE_DIR}/${component_name})

    # Define the input/output of the configuration for the component "meta" header
    set(meta_header_in ${AEROZEN_CMAKE_DIR}/aerozen_auto_headers.hh.in)
    ...
    set(config_header_install_dir ${AEROZEN_INCLUDE_INSTALL_DIR_FULL}/${PROJECT_INCLUDE_DIR})

    # Define the input/output of the configuration for the core "meta" header
    set(meta_header_in ${AEROZEN_CMAKE_DIR}/aerozen_auto_headers.hh.in)
    set(meta_header_out ${CMAKE_CURRENT_BINARY_DIR}/../${AEROZEN_DESIGNATION}.hh)
  ...
  if(NOT aerozen_install_all_headers_COMPONENT)
    ...
    if(NOT EXISTS ${config_header_in})
      message(FATAL_ERROR
        ...
        "migrating to the use of aerozen-cmake?")
    endif()
