...
#################################################
# aerozen_add_component(<component>
...
#              and its target will be aerozen-<project><major_ver>-<component>
#
...
#                          inter-component dependencies in an aerozen package.
#
# [INCLUDE_SUBDIR]: Optional. If specified, the public include headers for this
#                   component will go into "aerozen/<project>/<subdirectory_name>/".
#                   If not specified, they will go into "aerozen/<project>/<component>/"
#
...
#     the core library.
#
# See the documentation of aerozen_create_core_library(~) for more information about
...
function(aerozen_add_component component_name)
  ...
  #------------------------------------
  # Parse the arguments
  cmake_parse_arguments(aerozen_add_component "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(POLICY CMP0079)
    cmake_policy(SET CMP0079 NEW)
  endif()

  if(aerozen_add_component_SOURCES)
    set(sources ${aerozen_add_component_SOURCES})
  elseif(NOT aerozen_add_component_INTERFACE)
    message(FATAL_ERROR "You must specify SOURCES for aerozen_add_component(~)!")
  endif()

  if(aerozen_add_component_INCLUDE_SUBDIR)
    set(include_subdir ${aerozen_add_component_INCLUDE_SUBDIR})
  else()
    set(include_subdir ${component_name})
  endif()

  if(aerozen_add_component_INTERFACE)
  ...
  if(aerozen_add_component_GET_TARGET_NAME)
    set(${aerozen_add_component_GET_TARGET_NAME} ${component_target_name} PARENT_SCOPE)
  endif()
  ...
  _aerozen_add_library_or_component(
    LIB_NAME ${component_target_name}
    INCLUDE_DIR "${PROJECT_INCLUDE_DIR}/${include_subdir}"
    EXPORT_BASE AEROZEN_${AEROZEN_DESIGNATION_UPPER}_${component_name_upper}
    SOURCES ${sources}
    ${interface_option})

  if(aerozen_add_component_INDEPENDENT_FROM_PROJECT_LIB  OR
     aerozen_add_component_PRIVATELY_DEPENDS_ON_PROJECT_LIB)
    ...
    target_include_directories(${component_target_name}
      ${property_type}
        # This is the publicly installed aerozen/math headers directory.
        "$<INSTALL_INTERFACE:${AEROZEN_INCLUDE_INSTALL_DIR_FULL}>"
  ...
  _aerozen_handle_cxx_standard(aerozen_add_component
  ...
  # on the core library.
  if(aerozen_add_component_PRIVATELY_DEPENDS_ON_PROJECT_LIB)
  ...
  if(aerozen_add_component_INTERFACE_DEPENDS_ON_PROJECT_LIB)
  ...
  if(NOT aerozen_add_component_INDEPENDENT_FROM_PROJECT_LIB AND
     NOT aerozen_add_component_PRIVATELY_DEPENDS_ON_PROJECT_LIB AND
     NOT aerozen_add_component_INTERFACE_DEPENDS_ON_PROJECT_LIB)
  ...
  if(NOT aerozen_add_component_INDEPENDENT_FROM_PROJECT_LIB)

    # Add the core library as a cmake dependency for this component
    # NOTE: It seems we need to triple-escape "${aerozen_package_required}" and
    #       "${aerozen_package_quiet}" here.
    aerozen_string_append(${component_name}_CMAKE_DEPENDENCIES
      "if(NOT ${PKG_NAME}_CONFIG_INCLUDED)\n  find_package(${PKG_NAME} ${PROJECT_VERSION_FULL_NO_SUFFIX} EXACT \\\${aerozen_package_quiet} \\\${aerozen_package_required})\nendif()" DELIM "\n")

    # Choose what type of pkgconfig entry the core library belongs to
    set(lib_pkgconfig_type ${component_name}_PKGCONFIG_REQUIRES)
    if(aerozen_add_component_PRIVATELY_DEPENDS_ON_PROJECT_LIB
        AND NOT aerozen_add_component_INTERFACE_DEPENDS_ON_PROJECT_LIB)
      set(lib_pkgconfig_type ${lib_pkgconfig_type}_PRIVATE)
    endif()

    aerozen_string_append(${lib_pkgconfig_type} "${PKG_NAME} = ${PROJECT_VERSION_FULL_NO_SUFFIX}")

  endif()

  if(aerozen_add_component_DEPENDS_ON_COMPONENTS)
    aerozen_string_append(${component_name}_CMAKE_DEPENDENCIES
      "find_package(${PKG_NAME} ${PROJECT_VERSION_FULL_NO_SUFFIX} EXACT \\\${aerozen_package_quiet} \\\${aerozen_package_required} COMPONENTS ${aerozen_add_component_DEPENDS_ON_COMPONENTS})" DELIM "\n")
  endif()

  #------------------------------------
  # Set variables that are needed by cmake/aerozen-component-config.cmake.in
  ...
  # ... and by cmake/pkgconfig/aerozen-component.pc.in
  ...
  _aerozen_create_cmake_package(COMPONENT ${component_name})

  # Generate and install the pkgconfig information for this component
  _aerozen_create_pkgconfig(COMPONENT ${component_name})
  ...
  if(NOT all_known_components)
    set_property(TARGET ${PROJECT_LIBRARY_TARGET_NAME}-all
      PROPERTY INTERFACE_AEROZEN_ALL_KNOWN_COMPONENTS "${component_target_name}")
  else()
    set_property(TARGET ${PROJECT_LIBRARY_TARGET_NAME}-all
      PROPERTY INTERFACE_AEROZEN_ALL_KNOWN_COMPONENTS "${all_known_components};${component_target_name}")
  endif()
endfunction()
