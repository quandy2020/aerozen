...
# ----------------
#
# _aerozen_setup_packages
#
# Sets up package information for a Aerozen library project.
# This function is internal to aerozen-cmake.
#
# aerozen_create_packages
#
# Creates a package for a Aerozen library project
#
#===============================================================================
...
macro(_aerozen_setup_packages)
  ...
  if(CPACK_GENERATOR)
    ...
    configure_file("${AEROZEN_CMAKE_DIR}/cpack_options.cmake.in"
  ...
  set(AEROZEN_INCLUDE_INSTALL_DIR "${CMAKE_INSTALL_INCLUDEDIR}")
  set(AEROZEN_INCLUDE_INSTALL_DIR_POSTFIX "aerozen/${AEROZEN_DESIGNATION}${PROJECT_VERSION_MAJOR}")
  set(AEROZEN_INCLUDE_INSTALL_DIR_FULL    "${AEROZEN_INCLUDE_INSTALL_DIR}/${AEROZEN_INCLUDE_INSTALL_DIR_POSTFIX}")
  set(AEROZEN_DATA_INSTALL_DIR_POSTFIX "aerozen/${PROJECT_NAME_LOWER}")
  set(AEROZEN_DATA_INSTALL_DIR         "${CMAKE_INSTALL_DATAROOTDIR}/${AEROZEN_DATA_INSTALL_DIR_POSTFIX}")
  set(AEROZEN_LIB_INSTALL_DIR ${CMAKE_INSTALL_LIBDIR})
  set(AEROZEN_BIN_INSTALL_DIR ${CMAKE_INSTALL_BINDIR})
  ...
  configure_file(
    "${AEROZEN_CMAKE_DIR}/cmake_uninstall.cmake.in"
...
macro(aerozen_create_packages)

  #============================================================================
  # Load platform-specific build hooks if present.
  _aerozen_load_build_hooks()
...
#################################################
# _aerozen_create_pkgconfig([COMPONENT <component>])
#
...
# component's name.
#
# NOTE: This will be called automatically by aerozen_create_core_library(~) and
#       aerozen_add_component(~), so users of aerozen-cmake should not call this
#       function.
#
# NOTE: For aerozen-cmake developers, the variables needed by aerozen.pc.in or
#       aerozen-component.pc.in MUST be set before calling this function.
#
# Create a pkgconfig file for your target, and install it.
function(_aerozen_create_pkgconfig)
  ...
  #------------------------------------
  # Parse the arguments
  cmake_parse_arguments(_aerozen_create_pkgconfig "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  #------------------------------------
  # Choose which input file to use
  if(_aerozen_create_pkgconfig_COMPONENT)
    set(pkgconfig_input "${AEROZEN_CMAKE_DIR}/pkgconfig/aerozen-component.pc.in")
    set(target_name ${PROJECT_LIBRARY_TARGET_NAME}-${_aerozen_create_pkgconfig_COMPONENT})
  else()
    set(pkgconfig_input "${AEROZEN_CMAKE_DIR}/pkgconfig/aerozen.pc.in")
  ...
  include("${AEROZEN_CMAKE_DIR}/JoinPaths.cmake")
  join_paths(PC_LIBDIR "\${prefix}" "${CMAKE_INSTALL_LIBDIR}")
  join_paths(PC_INCLUDEDIR "\${prefix}" "${CMAKE_INSTALL_INCLUDEDIR}" "${AEROZEN_INCLUDE_INSTALL_DIR_POSTFIX}")
...
#################################################
# _aerozen_create_cmake_package([COMPONENT <component>]
...
#       function.
#
# NOTE: For aerozen-cmake developers, some of the variables needed by
#       aerozen-config.cmake.in or aerozen-component-config.cmake.in MUST be
...
function(_aerozen_create_cmake_package)
  ...
  #------------------------------------
  # Parse the arguments
  cmake_parse_arguments(_aerozen_create_cmake_package "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(_aerozen_create_cmake_package_COMPONENT AND _aerozen_create_cmake_package_ALL)
    message(FATAL_ERROR
      "_aerozen_create_cmake_package was called with both ALL and COMPONENT "
      "specified. This is not allowed!")
  endif()

  #------------------------------------
  # Set configuration arguments
  if(_aerozen_create_cmake_package_COMPONENT)

    set(component ${_aerozen_create_cmake_package_COMPONENT})
    set(target_name ${PROJECT_LIBRARY_TARGET_NAME}-${component})
    set(aerozen_config_input "${AEROZEN_CMAKE_DIR}/aerozen-component-config.cmake.in")
    set(simple_import_name ${component})

  elseif(_aerozen_create_cmake_package_ALL)

    set(aerozen_config_input "${AEROZEN_CMAKE_DIR}/aerozen-all-config.cmake.in")
    ...
    set(target_name ${PROJECT_LIBRARY_TARGET_NAME})
    set(aerozen_config_input "${AEROZEN_CMAKE_DIR}/aerozen-config.cmake.in")
    set(simple_import_name core)

  endif()

  if(_aerozen_create_cmake_package_LEGACY_PROJECT_PREFIX)

    set(LEGACY_PROJECT_PREFIX ${_aerozen_create_cmake_package_LEGACY_PROJECT_PREFIX})
  ...
  # This gets used by the aerozen-*.config.cmake.in files
  set(target_output_filename ${target_name}-targets.cmake)
  set(aerozen_config_output "${PROJECT_BINARY_DIR}/cmake/${target_name}-config.cmake")
  set(aerozen_version_output "${PROJECT_BINARY_DIR}/cmake/${target_name}-config-version.cmake")
  set(aerozen_target_ouput "${PROJECT_BINARY_DIR}/cmake/${target_output_filename}")

  # NOTE: Each component needs to go into its own cmake directory in order to be
  # found by cmake's native find_package(~) command.
set(aerozen_config_install_dir "${AEROZEN_LIB_INSTALL_DIR}/cmake/${target_name}")
  set(aerozen_namespace ${PROJECT_LIBRARY_TARGET_NAME}::)

  set(import_target_name ${aerozen_namespace}${target_name})
  set(simple_import_name ${aerozen_namespace}${simple_import_name})

  # Configure the package config file. It will be installed to
  # "[lib]/cmake/aerozen-<project><major_version>/" where [lib] is the library
  # installation directory.
  configure_package_config_file(
    ${aerozen_config_input}
    ${aerozen_config_output}
    INSTALL_DESTINATION ${aerozen_config_install_dir}
    PATH_VARS AEROZEN_LIB_INSTALL_DIR AEROZEN_INCLUDE_INSTALL_DIR_FULL)
  ...
  install(
    FILES
      ${aerozen_config_output}
      ${aerozen_version_output}
    DESTINATION ${aerozen_config_install_dir}
    COMPONENT cmake)

  # Create *-targets.cmake file for build directory
  export(
    EXPORT ${target_name}
    FILE ${aerozen_target_ouput}
    # We add a namespace that ends with a :: to the name of the exported target.
    # This is so consumers of the project can call
    #     find_package(aerozen-<project>)
    #     target_link_libraries(consumer_project aerozen-<project>::aerozen-<project>)
    # and cmake will understand that the consumer is asking to link the imported
    # target "aerozen-<project>" to their "consumer_project" rather than asking
    # to link a library named "aerozen-<project>". In other words, when
...
#
# This function is private to aerozen-cmake.
function(_aerozen_load_build_hooks)

  if(ARGV0)
    set(hook_dir ${ARGV0})
  else()
    set(hook_dir "${PROJECT_SOURCE_DIR}/cmake/aerozen-packager-hooks")
  endif()
