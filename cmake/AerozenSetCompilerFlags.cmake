#.rst
# AerozenSetCompilerFlags
# -------------------
#
# _aerozen_set_compiler_flags()
#
# Sets up compiler flags for a aerozen library project
#
#===============================================================================
...
macro(_aerozen_set_compiler_flags)

  option(USE_AEROZEN_RECOMMENDED_FLAGS "Build project using the compiler flags recommended by the aerozen developers" ON)
  ...
  if(GCC_OR_CLANG)
    ...
    if(USE_AEROZEN_RECOMMENDED_FLAGS)
      _aerozen_setup_gcc_or_clang()
    endif()
...
# Internal to aerozen-cmake.
macro(_aerozen_setup_unix)
  # No custom setup for Unix systems
endmacro()

#################################################
# Internal to aerozen-cmake.
macro(_aerozen_setup_apple)
...
# Internal to aerozen-cmake.
macro(_aerozen_setup_gcc_or_clang)

  if(aerozen_configure_build_EXPOSE_SYMBOLS_BY_DEFAULT)
  ...
  _aerozen_filter_valid_compiler_options(
    CUSTOM_ALL_FLAGS
...
# Internal to aerozen-cmake.
macro(_aerozen_set_sse_flags)

  message(STATUS "\n-- Searching for host SSE information")
  if(NOT EXISTS "${AEROZEN_CMAKE_DIR}/AerozenCheckSSE.cmake")
    message(STATUS "AerozenCheckSSE.cmake not found, skipping SSE auto-detection")
  else()
    include(AerozenCheckSSE)
...
macro(_aerozen_setup_msvc)
  ...
  if(USE_AEROZEN_RECOMMENDED_FLAGS)
    ...
    option(AEROZEN_MSVC_WPO "Enable Whole Program Optimization on MSVC" ON)
    if(AEROZEN_MSVC_WPO)
  ...
  # should ONLY be done if they do not intend to use the aerozen library as
  # part of a plugin-based framework.
  option(AEROZEN_USE_STATIC_RUNTIME "Use the static runtime (strongly discouraged)" OFF)
  ...
  if(BUILD_SHARED_LIBS)
    ...
    # is turned on.
    set(AEROZEN_USE_STATIC_RUNTIME OFF CACHE BOOL "Use the static runtime (strongly discouraged)" FORCE)
  endif()

  if(AEROZEN_USE_STATIC_RUNTIME)
