...
#   AEROZEN_DESIGNATION
#   AEROZEN_DESIGNATION_LOWER
#   AEROZEN_DESIGNATION_UPPER
#   PKG_NAME
...
#################################################
# Initialize the Aerozen project
macro(aerozen_configure_project)
  ...
  #------------------------------------
  # Parse the arguments
  _aerozen_cmake_parse_arguments(aerozen_configure_project "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  ...
  if(aerozen_configure_project_VERSION_SUFFIX)
    set(PROJECT_VERSION_SUFFIX ${aerozen_configure_project_VERSION_SUFFIX})
  endif()

  #============================================================================
  # Extract the designation
  #============================================================================
  set(AEROZEN_DESIGNATION ${PROJECT_NAME})
  # Remove the leading project prefix ("aerozen-" by default)
  set(PROJECT_PREFIX "aerozen")
  string(REGEX REPLACE "${PROJECT_PREFIX}-" "" AEROZEN_DESIGNATION ${AEROZEN_DESIGNATION})

  # Remove the trailing version number
  string(REGEX REPLACE "[0-9]+" "" AEROZEN_DESIGNATION ${AEROZEN_DESIGNATION})

  #============================================================================
  # Set project variables
  #============================================================================

  if(aerozen_configure_project_NO_PROJECT_PREFIX)
    set(PROJECT_NAME_NO_VERSION ${AEROZEN_DESIGNATION})
  else()
    set(PROJECT_NAME_NO_VERSION "${PROJECT_PREFIX}-${AEROZEN_DESIGNATION}")
  endif()
  ...
  string(TOLOWER ${AEROZEN_DESIGNATION} AEROZEN_DESIGNATION_LOWER)
  string(TOUPPER ${AEROZEN_DESIGNATION} AEROZEN_DESIGNATION_UPPER)

  string(SUBSTRING ${AEROZEN_DESIGNATION} 0 1 AEROZEN_DESIGNATION_FIRST_LETTER)
  string(TOUPPER ${AEROZEN_DESIGNATION_FIRST_LETTER} AEROZEN_DESIGNATION_FIRST_LETTER)
  string(REGEX REPLACE "^.(.*)" "${AEROZEN_DESIGNATION_FIRST_LETTER}\\1"
         AEROZEN_DESIGNATION_CAP "${AEROZEN_DESIGNATION}")

  set(PROJECT_EXPORT_NAME ${PROJECT_NAME_LOWER})
  set(PROJECT_LIBRARY_TARGET_NAME ${PROJECT_NAME_LOWER})

  if(aerozen_configure_project_REPLACE_INCLUDE_PATH)
    set(PROJECT_INCLUDE_DIR ${aerozen_configure_project_REPLACE_INCLUDE_PATH})
  else()
    set(PROJECT_INCLUDE_DIR aerozen/${AEROZEN_DESIGNATION})
  endif()
  ...
  if (DEFINED aerozen_configure_project_CONFIG_EXTRAS)
    list(APPEND extras ${aerozen_configure_project_CONFIG_EXTRAS})
  endif()
  ...
  # Do this after _aerozen_setup_packages() to ensure GNUInstallDirs has been called
  ...
  foreach(extra ${extras})
    _aerozen_assert_file_exists("${extra}"
      "aerozen_configure_project() called with extra file '${extra}' which does not exist")
    _aerozen_stamp("${extra}")

    # expand template
    _aerozen_string_ends_with("${extra}" ".cmake.in" is_template)
    if(is_template)
      ...
      configure_file(
        "${extra}"
        ${CMAKE_CURRENT_BINARY_DIR}/aerozen-cmake/${extra_filename}
        @ONLY
      )
      set(extra
        "${CMAKE_CURRENT_BINARY_DIR}/aerozen-cmake/${extra_filename}")
    ...
    _aerozen_string_ends_with("${extra}" ".cmake" is_cmake)
    if(is_cmake)
      install(FILES
        ${extra}
      ...
      message(FATAL_ERROR "aerozen_configure_project() the CONFIG_EXTRAS file '${extra}' "
...
#################################################
# Check the OS type.
macro(aerozen_check_os)

  # CMake does not distinguish Linux from other Unices.
  string(REGEX MATCH "Linux" AEROZEN_OS_LINUX ${CMAKE_SYSTEM_NAME})
  # Nor *BSD
  string(REGEX MATCH "BSD" AEROZEN_OS_BSD ${CMAKE_SYSTEM_NAME})
  # Or Solaris. I'm seeing a trend, here
  string(REGEX MATCH "SunOS" AEROZEN_OS_SOLARIS ${CMAKE_SYSTEM_NAME})

  # Windows is easy (for once)
  if(WIN32)
    set(AEROZEN_OS_WIN TRUE BOOL INTERNAL)
  endif()

  # Check if it's an Apple OS
  if(APPLE)
    # Check if it's OS X or another MacOS (that's got to be pretty unlikely)
    string(REGEX MATCH "Darwin" AEROZEN_OS_OSX ${CMAKE_SYSTEM_NAME})
    if(NOT AEROZEN_OS_OSX)
      set(AEROZEN_OS_MACOS TRUE BOOL INTERNAL)
    endif()
  endif()
